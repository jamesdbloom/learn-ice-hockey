...
>
> **Related:** [Breakouts](breakouts.md) · [Neutral Zone Systems](neutral_zone_systems.md) · [Zone Entries](zone_entries.md) · [Defensive Zone Coverage](defensive_zone_coverage.md) · [Body Contact and Battles](body_contact_and_battles.md) · [Game Management](game_management.md) · [Center](positions/center.md) · [Winger](positions/winger.md) · [Defender](positions/defender.md)
...
**Say it out loud on the bench and on the ice.** "I've got F1" or "I'm high" takes half a second and prevents the most expensive forechecking error there is — two players doing the same job while nobody does the other one. See [On-Ice Communication](on_ice_communication.md).
...
See [Body Contact and Battles](body_contact_and_battles.md) for the mechanics of angling, pinning and stick lifts, and [Time and Space](time_and_space.md) for the general principle of taking space away.
...
- Be realistic about fitness. An aggressive forecheck is a conditioning decision as much as a tactical one. See [Conditioning and Recovery](conditioning_and_recovery.md).
...
Full detail on the counters lives in [Breakouts](breakouts.md) and [Neutral Zone Systems](neutral_zone_systems.md). The skill of reading the shape before the puck arrives is covered in [Scanning and Anticipation](scanning_and_anticipation.md).
...
- **Forechecking a dump-in and a turnover the same way.** Off a dump-in you have a head start and can be aggressive. Off a turnover you are behind the play and should be recovering shape, not hunting.
- **Dumping the puck straight behind the net in a trapezoid league.** That is precisely the area their goalie is allowed to play. Aim for the corners.
...
7. **The trapezoid is why dump-and-chase works.** In the NHL, KHL and IIHF play the goalie may only play the puck behind the goal line inside it, so dump into the corners — never straight behind the net. Most rec leagues have no trapezoid, which changes the plan entirely.
8. **Off a dump-in you are hunting; off a turnover you are recovering.** Leave before the puck does on a dump-in. After a turnover, the closest player pressures and everyone else finds a layer rather than chasing.
