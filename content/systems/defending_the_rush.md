...
Rule: Make contact with the puck first and a tripping minor replaces the penalty shot — NHL 57.3 and IIHF 24.8, same words. USA Hockey writes no puck-first carve-out: 616(a) counts a foul diagonally from behind too, and lets the referee assess an optional minor
Rule: No book outside NHL Rule 57 caps a reckless trip at a minor — IIHF 57.3/57.4 reach a major and a major plus game misconduct, and 57.4 sits on Table 7, so a first major can end your game; USA Hockey 639(b) makes that pair mandatory and 639(c) a match penalty; Hockey Canada 8.6(b) mandatory on injury
...
Rule: Under the IIHF book "from behind" has one exception — 24.8 criterion (II) carves out a foul committed from the front by the goalkeeper on a breakaway
...
**Two carve-outs in the same rule that change what you actually do:**
...
- **Reaching around an attacker from behind on a breakaway.** Under NHL Rule 57.3 that is a penalty shot, and if your goalie is pulled it is an awarded goal — and under the IIHF, USA Hockey and Hockey Canada books a *reckless* trip is not capped at a minor either.
- **Reading "puck first" as a licence.** It saves you the penalty shot under the NHL and IIHF books and nothing more — USA Hockey writes no such carve-out at all, and no book anywhere caps a deliberate attempt to injure at a minor (NHL Rule 21.1).
