...
Rule: Neither the IIHF book nor the In-House Rules name a body-checking age — ask your league. IIHF 101.1 allows a bodycheck in women's hockey only with "a clear intention of playing the puck or attempting to 'gain possession' of the puck"; an illegal hit is a minor, or a major plus a game misconduct
...
Rule: A late hit is not capped at a minor in any of the four books — NHL 56.4 a discretionary major, but its 56.5 game misconduct needs an injury; IIHF 56.5 and Hockey Canada 8.3(b) reach a major plus a game misconduct with nobody hurt; USA Hockey 640(g) the same, 640(h) a match, under 640(b–f)
```
...
Rule: There is no minor-penalty option for checking from behind — it is a major with an automatic game misconduct where the opponent "is unable to protect or defend himself" (NHL Rules 43.2–43.5) or where the hit "recklessly endangers the fouled Player" (IIHF Rules 43.2–43.3)
Rule: That onus is weighed, not absolute — NHL Rule 41.1 lets the referee consider whether the opponent made themself vulnerable and whether the check was avoidable
Rule: Where a player deliberately turns their back into a check the two books read differently — NHL 43.1: "no penalty shall be assessed"; IIHF 43.1: "no penalty for 'checking from behind' shall be assessed. However, other penalties may still be assessed"
...
**Quick flush.** The opposite instinct: attack them before they are organised. A team that has just stopped behind its own net has not yet settled who is going where, and that is the moment you are attacking. F1 forces the carrier out to one side — ideally onto their backhand — and once F1 has committed, F2 comes in on an angle and doubles it. **Both of you are arriving on a player facing their own end boards, so this is angling, not hitting:** take the puck, take the ice, or steer them off it, and never finish into their back. There is no minor for checking from behind in either book (NHL and IIHF 43.2), and what makes a hit that major is exactly the position a flush puts them in — NHL 43.3 reaches an opponent *"who is unable to protect or defend himself"*, IIHF 43.3 a hit that *"recklessly endangers the fouled Player"*. F3 and both defencemen drop off to take away the longer passes out. Of the three it commits the most players before the puck is contested — and it is the only one of them aimed at winning the puck outright rather than at meeting the attack later.
