...
**The other thing that qualifies everything here is which book your league runs.** Several of these tactics are shaped by rules that only two of the four books write. The bar on changing after your own icing, the refusal of a time-out after one, and the coincidental-minors 4-on-4 are NHL-and-IIHF answers; Hockey Canada is often a third answer rather than a second; and USA Hockey frequently writes no such rule at all. 🇬🇧 **In Britain the IIHF flags are yours** — and in many fixtures there is no overtime whatever, so read **Overtime** and **Shootouts** before you rely on anything in them.
