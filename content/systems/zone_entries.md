...
**Attack the space between two defenders rather than at one of them** — one defender has an easy job, two sharing a gap have a problem. Get to the line last yourself and let your support arrive first — at the line and onside, not across it — which is a coaching convention rather than a rule.
...
Key: All of that cost is defensive — a failed carry produces no shots of its own, and it gives up 0.40 shots on the opponent's next entry, the highest shots-against figure in Tulsky's table
...
**Read what that eight hundredths is made of, though, because all of it is defensive.** A failed carry produces no shots on the entry itself — the first column is 0.00 — so the whole net figure is what happens next. Tulsky's Table 3 breaks that down two columns further than the version above: after a failed carry the attacking team gets **0.31 shots for** on the next entry and gives up **0.40 against**, and **0.40 is the highest shots-against figure in the table** — higher than a dump-and-chase (0.39), a successful carry (0.37) or a dump-and-change (0.32). So a small net cost is not evidence that a turnover at the blue line is harmless going the other way. It is evidence that you win the puck back often enough to pay most of it off — measured in **shots**, a count that weights every shot the same however dangerous it was. Chatel, below, prices that same half in expected goals instead.
...
The finding has held up across other data sets. None of these is a peer-reviewed paper, none of the underlying tracking is published, and they are not all reached first-hand — where a figure comes through a restater rather than from its own author, the bullet says so. Treat all of it as indicative:

- **Corey Sznajder** — roughly **0.66 shots per carry-in** against **0.29 per dump-in**. **This is the one still reached at second hand**, and its two restaters date it differently: Hockey's Arsenal gives the tracking as 2014-16, while Chatel credits the same pair of figures to *"Corey Sznajder presentation at RITSAC 2018"*. No publication by Sznajder himself was located for this document.
- **Thibaud Chatel**, April 2022, reported **47%** of controlled attempts led to a shot against **18%** of dump-ins, *"excluding dump and change"*. **Note the population, because it is neither the NHL nor your league:** over **56,700 five-on-five zone entries**, *"built from manually tracking games in Switzerland National League since 2019"* — European professional hockey. Chatel himself treats the agreement with the North American numbers as the finding, calling the two-for-one gap *"pretty much universal, small ice or big ice, and no matter the stat you look at."*
...
Different trackers, different leagues, different eras, same answer — **for the comparison Table 1 makes: getting in with control against putting it in without.** What none of them reports is the separate **shots-for** value of a *failed* attempt, which is the row the attempt-inclusive 0.48 depends on. So the two-to-one shape is the part that has replicated; the 0.48 rests on Tulsky's Washington games and on nothing else quoted here. Chatel does measure the *other* half of the failed-attempt question — what giving the puck up costs you going the other way — and that comes next.

**Chatel wrote that post to test the risk side of the argument, which is the objection everything above attracts** — *yes, but a turnover at the blue line is a rush the other way.* He followed what happened in the **next sequence** after the attacking team lost the puck, priced it in [expected goals](../off-the-ice/how_to_watch_hockey.md#expected-goals-xg) — ⚠️ **his own model, not a public one**: *"[w]e are using our own xG model here, based on shots manually tracked in Switzerland since 2019"*, and he states that it carries variables for whether a shot came off the rush and whether it came through the royal road — the slot line down the middle of the ice, defined in [Language and Glossary](../foundation/language_and_glossary.md) — so *"any odd-men rush situations and plays against a non-set-up defense would be reflected in the xG"* — and found the scenarios barely separate: *"all scenarios (dump or controlled entry, successful or failed) give up on around 0.06 or 0.07 xG to the opponent"*, which he read as *"showing that losing the puck after a controlled entry is not riskier after all."* **Read the sentence after that one too, because it does not go entirely his way.** Once the chance of losing the puck immediately is factored in, *"a failed controlled entry is still the biggest threat, with a potential 0.015 xG against you, but very close to a failed dump that has a potential 0.013 xG against you."* So on his own numbers the failed carry **is** the worst of them — by two thousandths of an expected goal. His account of why the gap feels so much larger than it measures is the part worth carrying onto the ice: *"we might remember goals allowed on ugly turnovers at the blue line, but nobody think about the dozens of possessions just handed over to the other team when you are dumping the puck."* **This is a different league, a different measure and a different set of hands from Tulsky's tables, and it does not extend them** — it is a separate test of the same objection, and it came out the same direction.
...
Rule: Your own side of the line is a vertical plane — a skate lifted and held back over the neutral zone is onside (NHL Rule 83; IIHF Rule 83.1 and PWHL Rule 85.1, in the same words), but not under USA Hockey or Hockey Canada
Rule: A "skate" is the blade only — NHL Rule 83.1 since 2025-26, PWHL Rule 85.1, and the IIHF's 2026/27 book; the 2025/26 IIHF book carries no such definition. USA Hockey defines it more broadly: its Casebook (Rule 630, Situation 8) says "skate" means "the blade or the boot"
Rule: An airborne trail skate is offside under USA Hockey 630(a), whose Casebook Situation 7 answers this exact case, and Hockey Canada Rule 6.11 with its Interpretation 3 — the blade must have contact. CARHA 72(c) borrows "own side" without defining it, so it settles nothing either way
Rule: British hockey runs the IIHF book at every level, so the IIHF wording is the rule you play under — an Elite League game runs the 2026/27 edition, which its Casebook makes the ruling for every rule it does not mention, and it mentions no Rule 83; below that the edition is unsettled, so ask
...
- The **leading edge** is the near edge of the blue line as you approach it. The line is painted **twelve inches wide** (Rule 1.5), and that whole foot of it counts as "the line" — a skate touching any of it is onside.
...
- For 2025-26 the NHL clarified that **"a 'skate' is to be considered the blade of the skate only."** **The IIHF has since written the same sentence into its own Rule 83.1**: the **2026/27** book reads *"For the purposes of this rule, a 'skate' is to be considered the blade of the skate only"*, where the **2025/26** edition carried no such definition and said only "skates". The consequence cuts both ways — your boot overhanging the line is irrelevant, and a **blade** across the plane is offside however far back the boot still is. **The PWHL carries the same sentence at its Rule 85.1.** **USA Hockey defines the word more broadly, and in your favour** — its *Casebook* at Rule 630, Situation 8 rules that *"[t]he word 'skate' in the Rule refers to the blade or the boot of the skate"*, so a boot still touching the neutral zone keeps you onside there.

**The first three clauses are the IIHF's own words, not a borrowing.** IIHF Rule 83.1 reads: *"A Player is 'off-side' when both skates are completely over the leading edge of the blue line involved in the play. A Player is on-side when either of their skates are in contact with the blue line, or on their 'own side' of the line, at the instant the puck completely crosses the leading edge of the blue line. On their 'own side' of the line shall be defined by a 'plane' of the blue line which shall extend from the leading edge of the blue line upwards. If a Player's skate has yet to 'break the plane' prior to the puck completely crossing the leading edge, they are deemed to be 'on-side' for the purpose of the 'off-side' rule"*. **That is the 2025/26 book, and the 2026/27 book keeps every word of it while adding two things.** One is the blade-only sentence above, dropped into the middle of the on-side test. The other is a rewritten carve-out for the puck carrier: a player who crosses ahead of the puck is not offside *"provided they have 'possession and control' of the puck **with their stick** prior to **both skates completely** crossing the leading edge"*, where 2025/26 asked only for possession and control *"prior to their skates crossing the leading edge"*. **⚠️ Those two halves do not run the same way, so do not read the new edition as uniformly stricter.** Requiring the stick is stricter — push the puck over the line with a skate and the carve-out is gone. *"Both skates completely"* is more permissive than the older *"their skates"*, and it lines the carve-out up with the both-skates test in the sentence above it. Both changes move the IIHF's wording onto the NHL's, which already reads that way. **And the IIHF's own *Situation Handbook* puts the airborne case directly:** *"As the puck crosses the attacking blue line, an attacking player jumps into the air right at the blue line. Is this off-side?"* — the answer is that the jumping player's skates *"must be on its own side of the leading edge of the blue line and have yet to break the 'plane of the leading edge' prior to the puck crossing the leading edge"* (Situation 83.29, word-identical in both editions).
...
> **The third clause is NHL and IIHF, and both North American books reverse it.** Under **USA Hockey Rule 630(a)** a player is offside when they do *"not have skate contact with any part of the Neutral Zone or the blue line when the puck crosses the determining edge of the blue line"*, and the Note confirms that *"a player only needs to have skate contact with one skate with the blue line to be considered 'onside.'"* **Hockey Canada Rule 6.11 runs the same way**: *"Only the player's skate(s) that are in physical contact with the ice surface will be used in determining an off-side"*, and its **Interpretation 3 to Rule 6.11(a)** settles the case in one line — *"A player has one skate above the blue-line (not touching the ice) and one skate over the blue-line at the instant the puck completely crosses the blue-line. OFF-SIDE."* **USA Hockey's own *Casebook* answers this exact picture** — a player whose *"trailing skate is directly above the ice in the Neutral Zone (off the ice)"* is offside, because *"they must have at least one skate in contact with the ice"* (Rule 630, Situation 7). So an airborne trail skate is **onside under the NHL, the IIHF and the PWHL — whose Rule 85.1 is the NHL's wording in the feminine — and therefore in Britain, and offside under USA Hockey and Hockey Canada**. **CARHA's Rule 72(c) borrows the phrase *"own side"* without ever defining it, so it answers this neither way.** That is every book checked here that settles it in its rule text. How much North American rec and youth hockey runs on the two books that say no is a question about leagues rather than about books, and this document has no count for it, so ⚠️ **find out which book your league plays under.** This changes step 2 below.
...
Rule: Lifting the trail skate is legal under the NHL, the IIHF and the PWHL, so it is legal in Britain (IIHF Rule 83.1; PWHL Rule 85.1) — USA Hockey Rule 630(a) and Hockey Canada Rule 6.11 require skate contact, so keep the blade down there
```
...
2. As you reach it, **stop pushing with the trail leg** and let that blade glide so it stays touching the painted line — or, **under NHL, IIHF and PWHL rules**, lift it and hold it back over the neutral zone without breaking the plane. **In Britain that lift is legal**, because the IIHF book is the one in force at every level here (IIHF Rule 83.1). **Under USA Hockey and Hockey Canada the blade must stay in contact** (Rule 630(a); Rule 6.11), so keep that blade on the painted line.
...
- Because it is the **skates** that decide and not the body — and under the NHL, the PWHL and the IIHF's 2026/27 book, only the **blade** — you can be leaning most of your body over the line. Aggressive body position is free. **One thing here is not free, and it is your head.** That sentence is about the offside rule, which does not care where your body is; it says nothing about what is in front of it. You are arriving at top speed with your body, hands and stick already across the line, and that is where a defenceman standing up at their own blue line will be — [§11](#standing-up-at-the-line) is the same play written from their side. **A head up is the condition of the technique, not an extra.** [Body Contact and Battles](../technique/body_contact_and_battles.md) states that rule without exceptions: you cannot brace for a hit you cannot see, and a body that is surprised does not absorb contact.
- **Which rule book you are under changes step 2.** Gliding the blade along the painted line is legal everywhere. *Lifting* the trail skate is legal under the **NHL, the IIHF and the PWHL** — and since British hockey runs the IIHF book at every level, it is legal here. **USA Hockey Rule 630(a)** requires *"skate contact with any part of the Neutral Zone or the blue line"*, and **Hockey Canada Rule 6.11** counts *"only the player's skate(s) that are in physical contact with the ice surface"*, so a raised trail skate is offside under both. **CARHA's book borrows the phrase *"own side"* without defining it, so it answers this neither way.** That is every book checked here that settles it. How much North American rec and youth hockey runs on the two that say no is a question about leagues rather than about books, and this document has no count for it, so find out which book your league plays under — and under USA Hockey's or Hockey Canada's, keep that trail skate on the ice.
...
Rule: The zone must be empty before anyone re-enters — NHL 83.3(i), IIHF 83.3(I) and USA Hockey 630(d)(2) word it "at the same instant"; Hockey Canada 6.11(b)(ii) as a result, "completely clear of attacking players". Both cite the tag-up clause. Same requirement, not a permission
Rule: Two more books write the same tag-up duty — PWHL 85.3(i) in the NHL's "at the same instant" wording, and CARHA 72(e)(2) asking every attacker to make "skate contact with the blue line", its Note stating the result as Hockey Canada does: "completely clear of attacking players"
Action: Get everybody out at once — one straggler keeps the arm up in every book here, so shout it
```
...
- **First, check that you have a delayed offside at all.** Under **USA Hockey Rule 630(d)** the delayed offside and the tag-up exist only *"(For all Youth 15-Only levels and above, all Girls 16U levels and above, High School and Adult classifications **only**)"*. Below those levels there is no raised arm and no recovery — the whistle goes the moment you precede the puck in. How much North American minor hockey sits below that line is a question about leagues and age classifications rather than about books, and this document has no count for it, so ⚠️ **check your own classification**: below that line this whole subsection describes a rule you do not have, and the entry simply has to be right first time. **This restriction is USA Hockey's alone** — Hockey Canada's Rule 6.11(b) carries no classification limit, and neither the NHL's 83.3 nor the IIHF's does.
- **Tagging up needs actual skate contact with the line.** The vertical plane above does *not* apply here — the IIHF text says so in terms, *"'3D' judgement is not used in this situation"* (Rule 83.3(I)). **That is the British rule book being explicit about it**, and it is the sharper wording of the two: NHL 83.3(i) says only *"(skate contact with the blue line)"* and leaves the rest to interpretation. Either way, the airborne trail skate that keeps you onside on the entry does **not** clear you on a delayed offside. Touch the painted line.
- **Everybody has to be out before anyone re-enters — in every book here, though not all of them write it as an instant.** One straggler keeps the arm up everywhere. Where the books part is only in the wording, not the requirement: the **NHL** requires that *"all players of the offending team clear the zone at the same instant (skate contact with the blue line)"* (Rule 83.3(i)), the **IIHF** says the same in the same words, adding only the gloss that *"'3D' judgement is not used in this situation"* (Rule 83.3(I)), and **USA Hockey** puts the same instant in different words, and is the most explicit of the three — *"all attacking players are simultaneously clear of the attacking zone by making skate contact with the blue line, at the same instant"* (Rule 630(d)(2), the tag-up clause). **Hockey Canada words it as a result rather than as an instant, and it comes to the same thing.** Rule 6.11(b) asks that *"all attacking players clear the attacking zone by making skate contact with the blue-line, so that the attacking zone is completely clear of attacking players"* — skate contact from every attacker, and the zone empty before anyone re-enters. Its Interpretation 11 resolves that to *"the instant the zone is completely free of attacking players"*, and IIHF Situation 83.34 states the same test in the book that uses the phrase: a player has cleared *"the instant the skate makes contact with the blue line"*, and *"should another Team A player enter the attacking zone while A31 is airborne, the delayed off-side shall remain in effect."* **The PWHL and CARHA write the duty too:** PWHL Rule 85.3(i) in the NHL's own words, and CARHA Rule 72(e)(2) asking that *"all attacking players in the attacking zone … clear the attacking zone by making skate contact with the blue line"*, with the Note printed under that clause stating the result in the same words Hockey Canada uses — *"[t]he attacking zone must be completely clear of attacking players before a delayed off-side can be nullified with the puck still in the attacking zone"*. **Different wording, same requirement.** Play it as though the instant matters and you are legal under all of them. ⚠️ **Read that citation carefully if you go to look it up, because USA Hockey's Rule 630(d) carries two separate numbered lists and "630(d)(2)" therefore names two different rules.** The **tag-up clause** is the one quoted here, under the heading *"the delayed offside will be nullified if"*. The **stop-play clause** sits under *"If any of the following conditions are met, play shall be stopped"*, and its second item is the pressure trigger dealt with in the next subsection. **That is the same shape as Hockey Canada's Rule 6.11(b), and this page uses the same two labels for both books**; they are not either rulebook's own names for them. **USA Hockey's own *Casebook* does not disambiguate them either** — its Rule 630 Situations 35, 36 and 37 give the reference "630(d.2)" for the tag-up clause, while Situations 38 and 39 give the same reference for the stop-play clause. Nothing on the ice turns on this; it only means a reader looking the number up should read both lists. This is a communication job — see [On-Ice Communication](../foundation/on_ice_communication.md). Shout it.

#### Forechecking during a delayed offside

```facts
Never: Forecheck during a delayed offside. Hold at the line instead and wait for the arm to drop, whichever book you are under
Rule: Every book here whistles a delayed offside for touching the puck, and for going for a loose one — NHL and IIHF 83.3, PWHL 85.3, Hockey Canada 6.11(b)(ii) and CARHA 72(e)(2) ("while the puck is still in the attacking zone"). USA Hockey 630(d) is wider: any attempt on the puck, loose or not
Rule: NHL and IIHF 83.3 add two further delayed-offside whistle triggers — forcing the defending puck carrier deeper, and being about to make contact with him. PWHL 85.3 writes both too; USA Hockey reaches the first through 630(d)'s stop-play pressure limb and Casebook Situation 38
Rule: USA Hockey 630(d), Hockey Canada 6.11(b)(ii) (the stop-play clause) and CARHA 72(e)(2) write no about-to-make-contact trigger — nothing you can rely on stops play before you arrive, and the defenceman is not braced for you
...
Rule: USA Hockey's Casebook keeps its minor plus misconduct for a push "not near the boards" or a minimal-contact "pinch", and requires the major plus a game misconduct, or a match, "in every instance" a check from behind "recklessly endangers the opponent" — board contact or none (Rule 608 Situation 1)
...
Rule: Hockey Canada's checking-from-behind ceiling is not its major — 7.5(c) is a Match penalty for a hit from behind "into the boards or goal frame" leaving the player "unable to protect or defend themselves", and that is the hit a turned defenceman on the wall is exposed to
Rule: Hockey Canada's Match penalty is not confined to the boards — 7.5(c)'s second paragraph assesses one to any player who "deliberately attempts to or deliberately injures an opponent by checking them from behind", asking nothing about where the player lands
Rule: CARHA 53(a) ends your night either way — a Minor or a Major, each plus a Game Misconduct, for an intentional hit from behind "anywhere on the ice", and a Match penalty "could also be assessed"; 53(b) is a certain Match into the boards or net, no intent limb, its Note barring a lesser substitute
Action: Retreat to the line, tag up, come again
```

**Forechecking during a delayed offside is not a grey area — it is a whistle. Which acts bring that whistle is not the same in every book, though, and the one that matters most for your safety is written in only three of them.** Under the **NHL**, play stops if any attacker *"touches the puck, attempts to gain possession of a loose puck, forces the defending puck carrier further back into his own zone, or who is about to make physical contact with the defending puck carrier"* (Rule 83.3). **The IIHF's Rule 83.3 lists the same four in the same order**, differing only in pronoun — so that is the British rule too. **⚠️ The fourth trigger — the whistle for a hit that has not landed yet — is not written in the other three books.** **USA Hockey Rule 630(d)** stops play only where *"(1) An attacking player touches the puck. (2) An attacking player attempts to gain possession of the puck or continues to apply pressure to the defending puck carrier."* Its *Playing Rules* write no about-to-make-contact condition at all. **But do not read the forcing-him-deeper trigger as NHL and IIHF property either.** USA Hockey's pressure limb is *wider* than the NHL's — pressure that never drives the carrier deeper is still a whistle there — and its separate *Official Rules and Casebook* states the forcing limb in terms. **Casebook Rule 630, Situation 38** answers that a defender may carry the puck anywhere in his own zone *"as long as they are not being forced deeper into their zone by an attacking player"*, and directs that the linesperson *"should be alert to stop play as soon as the defending puck carrier is forced back further into their Defending Zone or as soon as contact, however slight, is made with the puck or puck carrier."* **So the trigger that is restricted is the imminent-hit whistle, not the forcing one — and the books that write it are the NHL, the IIHF and the PWHL.** The Casebook does lean toward stopping play early where an offside attacker chases a loose puck — Situation 39 says *"The potential for unnecessary body contact should not be allowed in a situation where an infraction (offside) has already occurred"* — but that answers a player pursuing the puck, which is already a printed trigger under 630(d), and it gives you nothing to time a hit against. **Hockey Canada Rule 6.11(b)(ii)** (the stop-play clause) is shorter again — *"If any attacking player touches the puck or attempts to gain possession of a loose puck while the puck is still in the attacking zone, the Linesperson will stop play"* — no pressure limb, no contact limb, and a locative qualifier only CARHA carries as well. **CARHA Rule 72(e)(2) is the same clause in the same shape**, and its only stop-play trigger is the puck: *"If any attacking player touches the puck or attempts to gain possession of a loose puck while the puck is still in the attacking zone, play shall be stopped."* No pressure limb, no contact limb, there either. **⚠️ So under USA Hockey, Hockey Canada and CARHA, do not close on the defending carrier expecting the delayed-offside whistle to arrive first. Nothing you can rely on stops play before you arrive** — Hockey Canada and CARHA write no trigger at all short of the puck, and USA Hockey's is a linesperson's judgement of pressure, not of an imminent hit, so you cannot time your closing speed to it. The arm being up is not a stoppage, he is still playing the puck, and he is not braced for a hit. **⚠️ And name the hit you are lining up, because all three of those books do.** A defenceman turned to retrieve a puck in his own end has his back to you and his head over it, so hitting him there is **checking from behind**. **USA Hockey Rule 608**'s Note reaches the act directly — *"Checking from Behind occurs when a check is delivered to a player directly from behind, or diagonally from behind"*, and *"This includes body checking or pushing an opponent from behind in open ice or directly into the boards or goal frame"* — with **608(a)** writing *"[a] minor plus a misconduct penalty, or a major plus a game misconduct penalty"* and **608(b)** a major plus a game misconduct for a check that *"recklessly endangers an opponent, or causes them to go head first into the boards or goal frame"*. **⚠️ Do not price the hit you are lining up at that minor, because USA Hockey's own *Casebook* takes it off exactly this picture.** **Casebook Rule 608, Situation 1** keeps the minor plus misconduct for two things only — a push on an opponent *"skating ahead of them and not near the boards"*, and *"minimal body contact"* producing *"this 'pinch' against the boards"* — and then directs that *"[t]he major plus a game misconduct penalty, or match penalty, must be called"* in three instances, the first of which is your picture in terms: *"In every instance where a player forcefully checks an opponent who is standing along the boards (back toward the middle of the ice)"*, and the second *"[i]n every instance where a player is thrust head first into the boards or goal frame."* **The third does not need the boards at all** — *"[i]n every instance where the player recklessly endangers the opponent from a check from behind, regardless of whether or not board contact is made"* — so open-ice pressure on a turned defenceman is not a cheaper version of it. The Casebook also draws the area it is worried about: the *"danger zone"* is *"approximately ten feet out from the boards"*, and players checked from behind there who crash into the boards *"may be at high risk of receiving a serious and possibly life altering injury."* **Hockey Canada Rule 7.5 prints your exact picture** in the list it tells referees to enforce strictly, *"aware of the tragic consequences of spinal injuries"*: *"Where a player along the boards, with their head down looking for the puck or 'digging the puck out,' is hit from behind and driven head-first into the boards."* Its **7.5(a)** is a minor **plus a game misconduct** for an intentional hit from behind *"anywhere on the ice"*, and **7.5(b)** has two limbs. The first is a major plus game misconduct the referee **may** assess *"based on the degree of violence of the impact"* — **with nobody hurt**. The second **must** be assessed where a checking-from-behind foul that would otherwise be a minor injures the opponent. **The ejection does not wait for an injury.** **And 7.5(b) is not the ceiling either — 7.5(c) is a tier above it**, a **match penalty** for a hit from behind *"into the boards or goal frame, in such a way that the player is unable to protect or defend themselves"*, which is a defenceman turned to the wall for a puck, described. **CARHA's Rule 53 prices the same hit the same way, and its ceiling has no intent in it at all.** Its **53(a)** is *"[a] Minor penalty plus a Game Misconduct or a Major penalty plus a Game Misconduct at the discretion of the Referee, based on the degree of violence of the impact"* for a player who *"intentionally pushes, body checks, or hits an opposing player from behind, anywhere on the ice"* — so even the lighter of the two options there ends your night — and *"[i]f a player is injured, a Major penalty plus a Game Misconduct penalty must be assessed."* ⚠️ **The same limb prints a third sentence, and it has no boards in it:** *"A Match penalty could also be assessed under this rule."* So the open-ice version of the hit reaches the ejection too, not only the version into the wall. **Its 53(b) asks nothing about intent**: a **match penalty** where a player is *"pushed, hit or propelled in any manner from behind into the boards or goal net, in such a way that the player is unable to protect or defend themselves"*, which is the turned defenceman again. And the Note printed under it leaves the referee no cheaper option — *"Referees are instructed not to substitute other penalties when a player is checked from behind in any manner. This rule must be strictly enforced."* **So the three books that will not stop play before you arrive are the three that will take you off the ice after you do.** [Body Contact and Battles](../technique/body_contact_and_battles.md) covers how to arrive on a puck-retriever without becoming that. Hockey Canada's **off-side** rule answers the player who hits anyway too, but only after the fact and only in its own currency: Rule 6.11(d)(i) allows an **intentional off-side** against an attacker who *"deliberately plays or attempts to make a play on the puck **or against an opposing player** in a delayed off-side situation, knowing that they are off-side"*, and 6.11(d) puts the cost at a face-off *"at an end zone face-off spot, in the defending zone of the offending team"* — a territorial penalty rather than a minor, and nothing in *that* rule protects the man you hit. **The protection is Rule 7.5 above; the off-side rule was never the place to look for it.** **In every book here the instruction is identical: hold at the line, retreat, tag up, come again.**
...
Action: Drive the net to the edge of the blue paint and off the red line around it, not through either — a skate inside the paint, or on that line, is what risks the goal
Never: Meet a goal post head first, and never duck — head up, chin off your chest. A head-down, head-first hit on a post is the same injury as one on the boards, and can happen at walking speed
Rule: Keep off the red crease line at the paint's edge too — the NHL's and the IIHF's own reference tables both allow a goal on the line, but USA Hockey's 625(b) Note and Hockey Canada's Glossary count the crease lines as crease, and IIHF Rule 1.7 says the marked line belongs to it, so keep off it
Rule: The drive is not licence to hit the goaltender — contact you initiate with a goalie who is in his crease voids the goal, incidental or not (NHL and IIHF 69.3), and 69.2 penalises deliberate contact "whether or not the goalkeeper is inside or outside the goal crease"
Rule: USA Hockey 607(c) is not scoped to the crease — a check or charge on a goalkeeper "within the goal crease or privileged area" draws "a minor plus a misconduct or a major plus a game misconduct"; Hockey Canada 8.5(b) makes the major and game misconduct automatic for charging the goaltender
Rule: NHL and IIHF 69.4 also reaches the goaltender once he has left the crease to play the puck — an attacker whose "deliberate actions" prevent his return may be penalised for interference; the clause itself turns on intent, not contact
Never: Turn your back to the boards going down the wall — the wall, not the net; if you are tight to it and cannot get off it, take it with skates parallel, forearm and hip, head up and chin off your chest
...
- **Where that drive stops:** at the edge of the blue paint, not through it. **And arrive head up — never head first into a post, and never duck.** A head-down, head-first collision with a goal post is the same cervical-spine injury as one with the boards: USA Hockey's *Heads Up Hockey Program Guide* names *"the boards or goal post"* in one breath and says it *"can occur at walking speed"*, and USA Hockey's *Introduction To Body Contact*, under its *Basic Principles of Heads Up Hockey*, asks you to *"[h]it the boards or goal posts with an arm, a leg, or anything but your head first."* **That holds at every level and under every book — it is not a checking-league point, because a legal box-out moves you along the same path.** Standing in the goaltender's sightline from *outside* the crease, without contact, is normally legal and worth doing — it is a skate *inside* the paint that risks the goal. ⚠️ **Keep off the red line around the paint as well.** **IIHF Rule 1.7** has it that *"[t]he marked line belongs to the Goal Crease"*; **USA Hockey**'s Note to **Rule 625(b)** defines the crease area as the space outlined by the semi-circular crease lines *"(including crease lines)"*, and its *Casebook* answers the question in terms at *The Rink*, Situation 4 — *"Is the two inch red line outlining the goal crease considered to be a part of the goal crease? Yes."*; **Hockey Canada**'s Glossary says *"The lines of the crease are considered part of the crease."* **The NHL writes no such sentence in its rule text** — and **the reference tables of both the NHL and the IIHF allow the goal** for an attacker who *"plants himself on the crease line"*, at Situation 5 E in each (NHL Reference Table 14; IIHF Appendix IV, Table 16 in the 2025/26 edition and Table 14 in the 2026/27 one). ⚠️ **So the IIHF answers the same play in two directions inside one book**, and no book of the four ranks one layer of itself against another — a rule text against a Note, a Glossary or a reference table. Staying clear of the line is the conservative reading, and the one that never has to be argued. ⚠️ **The paint is only half of what the drive has to respect. The goaltender is the other half.** Contact you initiate with a goalie who is **in** his crease voids the goal whether it was incidental or not (**NHL and IIHF Rule 69.3**); with a goalie outside his crease, anything more than incidental contact does the same (**69.4**). ⚠️ **69.3 signposts one exception in its own sentence, and it is worth knowing:** **69.7** permits incidental contact and allows the goal *"[i]n a rebound situation, or where a goalkeeper and attacking player(s) are simultaneously attempting to play a loose puck, whether inside or outside the crease"* — the IIHF prints it word for word. **British readers should not plan around it, though:** the Motorpoint EIHL's own *Casebook* addition to the IIHF book writes the test flat — *"Contact in the blue paint area calls for disallowing a goal"* — and its interpretations supersede. **And the penalty runs separately from the goal:** **69.2** assesses one for *"intentional or deliberate contact with a goalkeeper, whether or not the goalkeeper is inside or outside the goal crease, and whether or not a goal is scored"*, and under **USA Hockey 607(c)** a player who *"body checks or charges a goalkeeper while the goalkeeper is within the goal crease or privileged area"* takes *"[a] minor plus a misconduct or a major plus a game misconduct penalty"* — **the upper half of which is an ejection.** ⚠️ **Hockey Canada goes further again, and keys it to no area of ice at all:** its **8.5(b)** leaves the major at the referee's discretion for ordinary interference with the goaltender and then adds a flat sentence — *"A Major penalty and Game Misconduct penalty will be assessed to any player who charges the goaltender"* — **an automatic ejection wherever he is standing**, and its bar for charging reaches a player who *"[b]uilds up speed by taking two or more strides immediately prior to making contact"* (Rule 7.4). [Offensive Zone Play](offensive_zone_play.md) sets out what each of the four books does with an attacker in the crease, and how far apart they are.
...
```facts
Position: A teammate without the puck, skating hard through the seam between the defencemen, at the net — to the edge of the blue paint, not into it
...
Never: Meet the post head first, or duck into it — head up, chin off your chest. A head-down, head-first hit on a goal post is the same injury as one on the boards, at walking speed
Rule: Committed stops at the paint's edge and off the red line around it — IIHF Rule 1.7, USA Hockey's Note to 625(b) and Hockey Canada's Glossary count the crease lines as crease, while the NHL's and the IIHF's own tables allow a goal on the line; a skate inside the paint risks the goal
Rule: Arriving hard is not licence to hit the goaltender — contact you initiate with a goalie in his crease voids the goal, incidental or not (NHL and IIHF 69.3), and 69.2 penalises deliberate contact "whether or not the goalkeeper is inside or outside the goal crease"
Rule: A goaltender is protected outside the crease too — IIHF 42.1, USA Hockey 607(d), Hockey Canada 8.5, CARHA's 52(b) Note and PWHL 42.1 all say he is not "fair game" out there; Hockey Canada 8.5(b) makes charging him an automatic major plus game misconduct wherever he is, and CARHA 52(b) in the crease
```
...
Middle drives only work if they are **committed**. A half-hearted middle drive is a player standing in the slot, which frees the defence rather than freezing it. **Committed still stops at the edge of the blue paint, and off the red line around it** — a skate inside the paint, or on that red line, is what risks the goal. **And committed still means head up.** Arriving at the net head down and meeting a post head first is the same cervical-spine injury as meeting the boards that way, and USA Hockey names *"the boards or goal post"* together for exactly that reason — hit either one with an arm, a leg, *"anything but your head first"*, and never duck. ⚠️ **And a committed drive is not licence to hit the goaltender:** contact you initiate with a goalie who is in his crease voids the goal whether or not it was incidental (**NHL and IIHF Rule 69.3**), and every book here prices a penalty separately from the goal — **NHL and IIHF 69.2** reach deliberate contact *"whether or not the goalkeeper is inside or outside the goal crease"*, **USA Hockey 607(c)** runs a check or charge up to an ejection while its 607(d) asks for a penalty *"in every case where an opposing player makes unnecessary contact with a goalkeeper"*, **Hockey Canada 8.5(b)** makes the ejection automatic for charging, and **CARHA 52(b)** and **PWHL 42.1** both write that same unnecessary-contact sentence — and CARHA's 52(b) then takes the discretion away as Hockey Canada's does, with a major plus a game misconduct that *"shall be assessed"* for charging a goalkeeper in the crease. The wide entry above sets out how the books split on the line itself, and quotes what each of them does to a player who hits the goaltender.
...
Rule: Every one of these drives ends at the edge of the blue paint and off the red line around it, and none is licence to hit the goaltender — contact you initiate with a goalie in his crease voids the goal (NHL and IIHF 69.3), and 69.2 reaches deliberate contact "inside or outside the goal crease"
Rule: USA Hockey 607(c) reaches a check or charge on a goalie "within the goal crease or privileged area" — "a minor plus a misconduct or a major plus a game misconduct" — Hockey Canada 8.5(b) makes that automatic for charging, and IIHF 42.1, CARHA 52(b) and PWHL 42.1 say he is not "fair game" outside it
Rule: CARHA 52(b) is mandatory too — a major plus a game misconduct that "shall be assessed" for charging a goalkeeper in the crease, and again for injuring any opponent by a charge with no location condition; its 52(a) Note counts more than two strides as a charge (CARHA-affiliated adult leagues)
```
...
**Every drive in every one of them ends in the same place, though** — at the edge of the blue paint, off the red line around it, and short of the goaltender. A triple drive puts three of you into that ice at once, which is exactly when the boundary gets forgotten. **Contact you initiate with a goalie who is in his crease voids the goal whether or not it was incidental** (**NHL and IIHF Rule 69.3**), and every book here prices a penalty separately from the goal — **NHL and IIHF 69.2** reach deliberate contact wherever the goaltender is standing, **USA Hockey 607(c)** runs a check or charge up to an ejection while its 607(d) asks for a penalty *"in every case where an opposing player makes unnecessary contact"*, **Hockey Canada 8.5(b)** makes the ejection automatic for charging, and **CARHA 52(b)** and **PWHL 42.1** both write that same unnecessary-contact sentence — and CARHA's 52(b) then takes the discretion away as Hockey Canada's does, with a major plus a game misconduct that *"shall be assessed"* for charging a goalkeeper in the crease. **The wide entry** above sets out where the books put the crease line, and what each of them does to a player who hits the goaltender.
...
Technique: Skates parallel to the wall, forearm and hip into it — not the point of your shoulder, not the whole shoulder if you can help it. Head up and chin off your chest
```
...
### The puck carrier should be the last to the line, with the supports waiting on it onside — a coaching convention, not a rule
...
**The IIHF's third option is a stop, and step 1 below says do not stop. Those are two different moments, not a disagreement.** Step 1 is scoped to approaching the line **alone or outnumbered**, where you are not going to out-stop the coverage. The IIHF's is scoped to an attacker already cut off from the net by one defender — and the manual attaches its own limit to it, in a separate *Quick Stop* entry: it works *"if the defender is not able to stop as quickly"*, and *"against a skilled defender, it will be necessary for the puck carrier to move laterally with the puck immediately after the quick stop in order to gain the necessary space."* ⚠️ **Read step 1 with its scope attached, not as a flat rule about stopping.**
...
Technique: Near the wall, skates parallel to the boards, take it on your forearm and hip — not the point of your shoulder, not the whole shoulder if you can help it. Head up and chin off your chest
...
Technique: Taking it against the boards, skates parallel to the wall, forearm and hip into it — not the point of your shoulder, not the whole shoulder if you can help it. Head up and chin off your chest
```
...
Risk: An overshot flip that crosses the goal line untouched from behind the centre red line is icing; whether being shorthanded saves you depends on your book, as Rules Primer sets out
Rule: A team below their opponent's on-ice strength cannot be called for icing (NHL 81.6) — but USA Hockey grants that only "For all Youth 15-Only and above, Girls 16U and above, High School and Adult classifications only" (Rule 624(b)(1))
Rule: NHL 81.6 withdraws the shorthanded icing exemption where a major expires with nobody on the penalty bench to exit — "Icing will be called" — but only until the next whistle. Rule 20.3 wants a substitute seated at a stoppage before that expiry, immediately where minors are stacked on the major
Rule: On a kill, a clear released before the penalty expires is not icing however late it lands — "prior to the penalty expiring, the icing infraction shall not apply" (NHL 81.6). USA Hockey grants it only from Youth 15-Only and Girls 16U up, judged "at the time the penalty expires" (624(b)(1))
Rule: 🇬🇧 Britain plays the IIHF book, whose 81.6 runs to two paragraphs with no major-penalty case and no classification limit — so a British kill keeps both the shorthanded icing exemption and the release-before-expiry relief
Rule: 🇬🇧 A British kill keeps the icing exemption, but that does not make an empty penalty box free — IIHF Rule 20.3 wants the substitute seated "immediately", and Rule 68.2 prices a failure at a bench minor, which is a 5-on-3 if the whistle comes while the major is still running
```
...
- **How:** open the blade, get under the puck, and aim to land it **past** the defence but **before** the goal line so it does not carry through for icing. Beware: a puck crossing the goal line untouched from behind the centre red line is **icing**. ⚠️ **Being shorthanded does not always save you.** USA Hockey grants that exemption *"(For all Youth 15-Only and above, Girls 16U and above, High School and Adult classifications only)"* (Rule 624(b)(1)), so at 14U and Girls 14U and below a shorthanded team is called for icing like anyone else; and NHL Rule 81.6 withdraws it from a team short by a *major* that has *"neglected to ensure there is a player on the penalty bench to exit upon the expiry of the penalty"* — *"Icing will be called."* **Read that limb to the end, because it bites at one moment only — the expiry — and then lifts at the next whistle:** *"[t]hey may substitute for this penalized player at the next stoppage of play"* — the same limit Rule 20.3 puts on the skater you are down. Rule 20.3 lets a team whose penalised player has been removed from the game or injured wait — it *"does not have to place a substitute player on the penalty bench immediately, but must do so at a stoppage of play prior to the expiration of the major penalty"* — so for most of a five-minute major the box may sit legitimately empty and the exemption still holds. ⚠️ **Not in every case, though: where that player also has minor penalties running on the clock alongside the major, the same rule requires the substitute *"immediately"*.** **The relief written into the same rule runs your way, too.** Icing on an expiring penalty is judged *"at the instant the penalty expires"*, and *"[s]hould the puck be released from the stick of the player shooting the puck down the ice prior to the penalty expiring, the icing infraction shall not apply"* (NHL 81.6; USA Hockey 624(b)(1) says the same in its own words, judging it *"at the time the penalty expires"* rather than *"at the instant"* — but it says so **inside that same classification-limited clause**, so the relief stops where the exemption does). **With two seconds left on the kill, fire it — if the exemption reaches your classification.** 🇬🇧 **Neither carve-out reaches a British reader.** IIHF Rule 81.6 runs to two paragraphs and stops — no major-penalty paragraph, no classification limit — so under the book British hockey is played on, both the exemption and that relief hold. ⚠️ **Read that as a deadline rather than an amnesty, though, because an empty penalty box is not free under the IIHF either.** Where NHL Rule 20.3 lets that team wait for a stoppage, **IIHF Rule 20.3 allows no delay at all** — the offending team *"must place a substitute in the penalty box immediately"* — and its **Rule 68.2** treats a failure as an illegal substitution *"calling for a bench minor penalty."* **The NHL's own 68.2 closes with that same sentence, so what divides the two books here is the deadline rather than the price** — and a whistle while the major is still running makes it a 5-on-3, the two minutes landing on a kill already down to four skaters. See the [Rules Primer](../foundation/rules_primer.md) for the full icing rule, including hybrid icing, and [Special Teams](special_teams.md) for the operative wording in all five books and the head count the exemption actually turns on.

### How the trapezoid shapes where you aim

```facts
Rule: Under the NHL and IIHF books a goalkeeper may not play the puck outside the designated area behind the net — a two-minute delay-of-game minor (NHL Rules 27.8 and 63.2(viii))
...
Rule: Chasing their goalie is not licence to hit them — IIHF and NHL 42.1, USA Hockey 607(d), Hockey Canada 8.5, CARHA 52(b) and PWHL 42.1 all say a goalkeeper is not "fair game" outside the crease, and Hockey Canada 8.5(b) makes a major plus a game misconduct automatic for charging the goaltender
...
Rule: CARHA 52(b) makes a major plus a game misconduct mandatory for charging a goalkeeper in his crease, and again for a charge that injures anyone, with no location condition on that half; its 52(a) Note counts more than two strides as a charge (CARHA-affiliated adult leagues)
...
**Where the trapezoid applies:** the NHL and IIHF play (IIHF Rule 27.7, with the area defined as 6.80 m along the goal line widening to 8.60 m at the boards). **The NHL's commonly given 2005-06 start date is in no rulebook held here** — its Rules 1.8, 27.8 and 63.2(viii) carry no date at all — so treat it as the received date rather than a checked one. **The KHL is commonly said to have one too, though that one is unverified here: no KHL rulebook was available to check it against.** **In England and Wales, treat it as unsettled.** The IIHF book is the one in force and the In-House Rules do not amend Rule 27, in the current 2026-27 edition or the 2025-26 one before it — but England Ice Hockey's own *Rules & Regulations* 22.3 say the restricted area is *"not currently enforced to allow ice rinks the necessary time to make the changes to implement this rule"*, and that edition is 2024-25 while both In-House editions are silent on it. For you as the attacking team the safe assumption runs the other way: plan as though their goalie **may** retrieve the puck. Ask your league, and look behind the net to see whether the lines are painted at all. **Neither North American governing body's book has one:** the *USA Hockey Playing Rules 2025-29* and the *Hockey Canada Playing Rules 2026-2028* mark no goalkeeper's restricted area and put no limit on *where* a goalkeeper may play the puck, so a league running on either book has none. **How much rec, beer-league and youth hockey that adds up to is a question about leagues rather than about books, and this document has no count for it.** Where there is no trapezoid a puck-handling goalie can go anywhere behind his goal line, which flips the advice: rim it hard and low so he cannot set up on it, or dump to the corner furthest from his stick hand. **Under either of those books the forecheck still pays, though**, because neither lets their goalkeeper smother a dumped puck out there: USA Hockey Rule 614(c) bars *freezing* it outside the *"goalkeeper's privileged"* area, which excludes the corners, and Hockey Canada's Interpretation 3 to Rule 10.1(a), clause v, assesses a minor with no warning once the goaltender has left the crease. **Both books have a clause written for exactly the play you are making, and both run your way.** Hockey Canada's **clause iv** of the same Interpretation names it: a goaltender who comes out *"to beat an attacking player to the puck and simply jumps on the puck, causing a stoppage of play"* takes a minor, and *"[n]o warning will be issued in this instance."* USA Hockey's *Casebook* works the identical facts at **Rule 614(c), Situation 6** — the goalkeeper rushes a loose puck inside their own privileged area to beat a lone attacker to it and falls on it — and answers **Yes**, a minor, because *"[t]he goalkeeper has an obligation to play the puck with their stick, just like any other player."* **Situation 12** adds the boundary: a cover made *"behind the goal line or outside of the privileged area"* is one where the minor *"must be assessed."* ⚠️ **Do not carry that to a goaltender who came out to make a *save*** — a cover taken after stopping a shot is a different case in both books, and one where the carve-outs run the other way. [Goaltender](../positions/goaltender.md) sets them out. So a dumped puck in the corner still has to be played rather than smothered, and the chase is what forces it. ⚠️ **Forcing it is not licence to hit them, and this part is not scoped to the books without a trapezoid — every book here says it in terms.** **USA Hockey Rule 607(d)**: *"A goalkeeper is NOT 'fair game' because they are outside the privileged area. A penalty for interference or charging should be called in every case where an opposing player makes unnecessary contact with a goalkeeper."* **Hockey Canada Rule 8.5** puts it in its own words and stretches it further — *"A Goaltender is not 'fair game' just because they are outside their goal crease. A penalty under this rule will be called where an opposing player makes unnecessary contact with the goaltender anywhere on the ice."* **The IIHF writes it twice.** **Rule 42.1**, inside the charging rule: *"A goalkeeper is not 'fair game' just because they are outside the goal crease area. The appropriate penalty should be assessed in every case where an opposing Player makes unnecessary contact with a goalkeeper."* **Rule 69.4 says it again in words of its own, not word for word** — *"outside the goal crease"* rather than *"the goal crease area"*, and *"an attacking Player"* rather than *"an opposing Player"*, which is the narrower of the two. Neither difference gives you a hit. ⚠️ **That is the sentence a British reader is playing under** — IHUK and BUIHA adopt the IIHF Rule Book *"as the foundation for the rules of the game to be applied in England, Wales, Scotland and Northern Ireland"*, and their In-House Rules amend Rule 69 only as to video review, leaving 42.1 and 69.4 untouched. **NHL 42.1** carries the same sentence in its own pronouns, as *"just because he is outside the goal crease area"*. **CARHA 52(b) and PWHL 42.1 print it too**, so a rec or PWHL reader gets no lighter a rule than anyone else.

**The books then diverge on the accident, and this document errs strict on purpose.** The IIHF and the NHL both add a permission with its limit attached inside the same sentence — *"However, incidental contact, at the discretion of the Referee, will be permitted when the goalkeeper is in the act of playing the puck outside their goal crease, provided the attacking Player has made a reasonable effort to avoid such contact"* (IIHF 42.1, quoted here; NHL 42.1 carries the same permission in all but its pronouns, writing *"outside his goal crease"*). **USA Hockey writes no such permission.** Its **Note 1 to 607(d)** sends *"any accidental or unavoidable contact that occurs with the goalkeeper"* to the interference rule, and *"[a]ny deliberate body contact or check that is delivered to the goalkeeper"* to charging. ⚠️ **So contact that is a referee's discretion under the IIHF and the NHL is a penalty under USA Hockey**, and the way to be right under all four is the same: make the effort to avoid, and do not treat a goalie outside his crease as a hittable player.

**The tier is the reason to care.** Under **USA Hockey 607(c)** a body check or charge on a goalkeeper inside the crease or privileged area is *"a minor plus a misconduct or a major plus a game misconduct"*, the upper half of which is an ejection. ⚠️ **Hockey Canada does not leave the worst version of it to the referee at all.** Its **8.5(b)** keeps the major discretionary for ordinary interference — *"A Major penalty and Game Misconduct penalty, at the discretion of the referee, based on the degree of violence of impact, may be assessed to any player who commits interference with the goaltender"* — and then takes the discretion away for the hit this section is about: *"A Major penalty and Game Misconduct penalty **will** be assessed to any player who charges the goaltender."* **That is an automatic ejection**, and two other places in the book say the same: **8.5(e)**, *"A Game Misconduct penalty must be assessed any time a Major penalty is assessed for interference with the goaltender"*, and **Interpretation 3 to Rule 7.4(b)**, which routes contact on a goaltender in the crease *"that could be described as charging"* to exactly that outcome — *"a Major penalty and Game Misconduct penalty for Interference must be assessed"*. ⚠️ **And Hockey Canada's bar for "charging" is low enough to reach an ordinary hard chase:** **Rule 7.4** counts a player who *"[b]uilds up speed by taking two or more strides immediately prior to making contact"*, as well as one who jumps, travels an excessive distance, or checks violently and unnecessarily. ⚠️ **CARHA can end your night two ways, and one of them has no crease in it.** Its **52(b)** takes the discretion away for charging a goalkeeper *"within the goal crease"*, and again for a charge that *"injures an opponent"* — that half carries no location condition at all. **Out of the crease the rule that reaches you is its 66, not its 52:** **66(b)** makes deliberate contact with a goalkeeper *"whether in or out of the goal crease"* a minor, with a major *"[a]t the discretion of the Referee"*, and **66(e)** makes that major mandatory for a player who *"injures an opponent by interference"*. ⚠️ **A discretionary major there is not the cheaper outcome it reads as, because in that book every major is an ejection:** **Rule 30(a)** rules a player assessed one *"off the ice for the remainder of the game (Major penalty plus Game Misconduct)"*, its only exception an accidental high stick. (CARHA-affiliated adult leagues.) ⚠️ **And the IIHF's own answer is not only its charging rule, because the route you arrive by decides which rule you are under.** A goaltender turned to the boards for a rimmed puck, with a chaser arriving, is a check from behind described — and **43.1** reaches a Player who *"hits an opponent from behind into the boards, the goal frame, or on open ice in any manner (i.e., high-sticking, cross-checking, etc.)"*, while **43.2** removes the bottom rung: *"[t]his infraction is considered serious and dangerous, therefore there is no option to award a minor penalty for 'Checking from Behind'."* **What 43.3 makes automatic is the game misconduct once the major is given, not the major** — the Referee still judges whether it was a check from behind and whether it recklessly endangered him, and the ceiling does not move, because the IIHF writes no match penalty anywhere. *(That Rule 43 reaches a goaltender is a reading of how the book is assembled rather than a sentence the IIHF prints: Rule 5.1 makes any reference to* "Player" *cover goalkeepers and 43.1 names the goal frame, but its Appendix II glossary defines a vulnerable position for* "a skater" *and its Situation Handbook prints no Situation under Rule 43 at all.)* **Arrive on the puck, not on the goalie.**
...
Rule: Hockey Canada 7.5(c) is a Match penalty for a hit from behind "into the boards or goal frame" leaving the player "unable to protect or defend themselves", and 7.5(b) reaches a major plus a game misconduct on the violence of the impact alone
Rule: The boards are not what makes a check from behind a Match penalty under Hockey Canada — 7.5(c)'s second paragraph assesses one to any player who "deliberately attempts to or deliberately injures an opponent by checking them from behind", wherever the retriever ends up
...
- **Name the hit before you arrive.** A defenceman who has turned to retrieve has his back to you and his head over the puck, and arriving through him is **checking from behind**. **USA Hockey's *Casebook*** (Rule 608, Situation 1) requires the major plus a game misconduct, or a match penalty, *"[i]n every instance where a player forcefully checks an opponent who is standing along the boards (back toward the middle of the ice)"*, and **Hockey Canada Rule 7.5** lists *"[w]here a player along the boards, with their head down looking for the puck or 'digging the puck out,' is hit from behind and driven head-first into the boards"* among the checks referees are told to enforce strictly, with **7.5(a)** a minor **plus a game misconduct** for an intentional hit from behind *"anywhere on the ice"*. ⚠️ **That minor is the floor, not the price.** **7.5(b)** lets the referee go straight to a major plus a game misconduct *"based on the degree of violence of the impact"* with nobody hurt, and **7.5(c)** assesses a **match penalty** for a hit from behind *"into the boards or goal frame, in such a way that the player is unable to protect or defend themselves"* — which is the retriever you are angling on, described. **And 7.5(c) has a second paragraph with no boards in it**, a match penalty for any player who *"deliberately attempts to or deliberately injures an opponent by checking them from behind"*. ⚠️ **And USA Hockey's half of this costs a second night:** the major it requires comes with a game misconduct, and Rule **404(b)** suspends any player who incurs one *"for the team's next game"* as well. The whole point of angling is that you take the escape away rather than arrive through him. [Body Contact and Battles](../technique/body_contact_and_battles.md) covers how to arrive.
...
Read these against the defender's own scale, which [Defending the Rush](defending_the_rush.md) covers: **two to three stick lengths is a normal neutral-zone gap**, about a stick and a half at the red line, and about a stick length is what a defender aims for at their own blue line. Those are coaching convention, and the published pages disagree with each other and with them — the only page found that anchors a distance to the centre red line prints two stick lengths there rather than a stick and a half, and a stick length at the blue line where a second page prints two. The tighter **one to one and a half stick lengths** you will also hear is not part of that owned scale — it is the coaching description of a *tight* gap carried by [Neutral Zone Systems](neutral_zone_systems.md), and it is a deliberate choice rather than the default.
...
- **The general target**, and the scale [Defending the Rush](defending_the_rush.md) covers, is **two to three stick lengths through the neutral zone, about a stick and a half at the red line, and about one stick length by your own blue line** — shrinking the whole way, never growing. Those distances are coaching convention, and the published pages disagree with each other and with them — the only page found that anchors a distance to the centre red line prints two stick lengths there rather than a stick and a half. But the number is less important than the principle: **your gap should be whatever lets you match their next change of speed.** That a change of speed is the harder thing to match is coaching craft rather than a measured finding — [Time and Space](../hockey-iq/time_and_space.md) carries the two coaching sources behind it, neither of which measures it.
...
- **Overshooting a flip dump — and then misjudging whether the kill is covered.** **Land the flip short of the goal line; and a man down, take the clear you are entitled to — declining one you could have made is the commoner error by far.** You may fire it the length of the ice, and a puck released before your penalty expires is not icing however late it lands. Behind that instruction: a puck crossing the goal line untouched from behind the centre red line is icing, and the shorthanded exemption is not universal — USA Hockey withholds it below Youth 15-Only and Girls 16U (Rule 624(b)(1)), and the NHL withdraws it where a major **expires** with nobody on the penalty bench to exit (Rule 81.6), **not for the rest of the major: that ban lifts at the next stoppage** (Rules 81.6 and 20.3). The bench may sit legitimately empty until that expiry unless minors are stacked on the major, when a substitute is required immediately (NHL Rule 20.3). 🇬🇧 The IIHF book, which British hockey is played on, carries neither carve-out — **but seat a substitute in the penalty box immediately when a major empties it, because there the failure itself is a bench minor** (IIHF Rules 20.3 and 68.2).
...
- **Dumping straight behind the net where the trapezoid applies.** Use the corners. Straight behind hands the goalie a legal touch and lets him set the puck up for his defenceman.
- **Arriving on the goalie instead of the puck.** **Take the puck, not the man** — a goaltender out of his crease playing a dumped puck is not a hittable player, and every book here says he is not *"fair game"* out there. **Hockey Canada 8.5(b) makes a major plus a game misconduct automatic for charging the goaltender — an ejection with no referee's discretion in it — and its charging bar reaches a player who builds up speed by taking *"two or more strides immediately prior to making contact"* (7.4)**, which is an ordinary hard chase described. **CARHA takes the same discretion away, and reaches further:** its **Rule 52(b)** assesses a major plus a game misconduct to a player who *"charges a goalkeeper while the goalkeeper is within the goal crease or who injures an opponent as a result of a charge"* — the half after the "or" has no crease condition and is not about goaltenders at all — and the Note to its **52(a)** counts *"more than two steps or strides"* as a charge, which is a hard chase described again. **Out of his crease, though, the rule that reaches you is CARHA's 66, not its 52:** **66(b)** makes deliberate contact with a goalkeeper *"whether in or out of the goal crease"* a minor, a major at the referee's discretion, and **66(e)** makes a major mandatory for a player who *"injures an opponent by interference"*. ⚠️ **A discretionary major there is no cheaper than an automatic one, because Rule 30(a) rules any player assessed a major off the ice for the rest of the game.** (CARHA-affiliated adult leagues only.) USA Hockey is stricter again on the accident: its Note 1 to 607(d) sends even *"accidental or unavoidable contact"* to the interference rule, where the IIHF and NHL permit incidental contact only while he is playing the puck and only if you made a reasonable effort to avoid it. ⚠️ **And under the IIHF book British hockey is played on, the route you arrive by decides which rule prices it:** a hit landed on a goaltender's back that puts him into his own net is checking from behind, and **IIHF 43.2** leaves *"no option to award a minor penalty for 'Checking from Behind'"* — 43.1 naming *"the goal frame"* among the places you may not hit a Player into. ⚠️ **And once he has that puck and is heading back to the crease, the foul protecting him no longer needs contact at all:** **NHL and IIHF Rule 69.4** lets an attacker be penalised for goalkeeper interference where deliberate actions stop him getting home — the clause itself turns on intent, not contact.
- **Coasting over the line on a delayed offside.** **Put a skate on the line — every attacker who is in the zone, and together — and you are legal everywhere.** Tagging up needs **skate contact with the blue line** in every book that writes the test. Being in the air over it does not clear you, and one player not tagging keeps the whole team frozen. The zone has to be empty before anyone re-enters, in every book here — four of them word it *"at the same instant"*, and Hockey Canada and CARHA state it as a result instead — the zone *"completely clear of attacking players"*, CARHA's in the Note under its clause — which comes to the same thing. (And check your book: under USA Hockey Rule 630(d) there is no tag-up at all below Youth 15-Only, Girls 16U, High School and Adult — the whistle goes immediately. Hockey Canada sets no such classification limit.)
- **Forechecking during a delayed offside, and expecting the whistle to protect you.** **Hold at the line and wait for the arm to drop — under any book.** Under the NHL and the IIHF, being *about to make physical contact* with the defending puck carrier is itself a whistle (Rule 83.3). **⚠️ Under USA Hockey Rule 630(d), Hockey Canada Rule 6.11(b)(ii) (the stop-play clause) and CARHA Rule 72(e)(2) it is not** — none of those three writes a contact trigger, so nothing you can rely on stops play before you arrive and the defenceman is not braced for you. Hockey Canada and CARHA write no pressure trigger at all; USA Hockey 630(d) does whistle an attacker who *"continues to apply pressure to the defending puck carrier"*, but that is a linesperson's judgement of pressure, not of an imminent hit. Touching the puck, or going for a loose one, stops play in every book here. **⚠️ And a defenceman turned to retrieve the puck has his back to you, so hitting him is checking from behind** — USA Hockey Rule 608 reaches the act *"in open ice or directly into the boards or goal frame"*, and its *Casebook* (Rule 608, Situation 1) requires the major plus a game misconduct, or a match penalty, rather than a minor, *"[i]n every instance where a player forcefully checks an opponent who is standing along the boards (back toward the middle of the ice)"*. Hockey Canada lists the head-down puck retrieval along the boards among the situations to be enforced strictly, attaches a game misconduct to even its minor (Rules 7.5(a) and 7.5(e)), and reaches a **match penalty** for a hit from behind *"into the boards or goal frame, in such a way that the player is unable to protect or defend themselves"* (7.5(c)) — whose second paragraph reaches one again, without the boards, for any player who *"deliberately attempts to or deliberately injures an opponent by checking them from behind"*. **CARHA sits in the same place**: its 53(a) carries a game misconduct on both of its options, and its 53(b) match penalty for a hit from behind *"into the boards or goal net"* leaving a player *"unable to protect or defend themselves"* asks nothing about intent, under a Note telling referees *"not to substitute other penalties"* — and its 53(a) adds that a match penalty *"could also be assessed under this rule"*, with no boards in it at all. [Forechecking during a delayed offside](#forechecking-during-a-delayed-offside) sets every tier out.
- **Retrieving the puck on a delayed offside with your head down.** Head up — the arm is not a shield. Where the books protect the defender at all they name the *"defending puck carrier"* (NHL and IIHF Rule 83.3, USA Hockey Rule 630(d)), so the protection lapses the instant you make the outlet pass, and Hockey Canada Rule 6.11(b)(ii) (the stop-play clause) and CARHA Rule 72(e)(2) write no pressure or contact trigger at all. A whistle for an imminent hit does not stop a body already committed at speed.
- **Driving the middle with your head down.** **Head up is the condition of the drive, not an extra.** The middle seam is the one route where two defencemen can both stand you up in open ice, and under the IIHF book a square check with no upward motion on a puck carrier skating head down **toward the checker** is not penalised as a check to the head (Rule 48.1). **⚠️ The sentence immediately before it in the same rule runs the other way and is the one written for you:** *"When a Player is skating with their head up, whether they are in possession of the puck and may reasonably be expecting impending contact, an opponent does not have the right to hit them to the head or neck."* It holds whether or not you have the puck, and a head up is what keeps that sentence yours. [Three things that do not bend](#three-things-that-do-not-bend-whichever-pattern-you-run) sets out how far the carve-out reaches, which the IIHF's own text leaves unresolved, and what the other books do instead.
- **Dragging the trail skate with your head down.** **Head up, because the drag puts your body, hands and stick over the line before your skate** — into the ice a defenceman standing up at their own line is defending. Aggressive body position is free under the offside rule and nowhere else — see [The trail-skate drag](#the-trail-skate-drag).
- **Driving the net *into* the blue paint, or into the goaltender, instead of to the edge of it — or arriving there head down.** **Head up, and never meet a post head first or duck into one:** a head-down, head-first hit on a goal post is the same cervical-spine injury as one on the boards, USA Hockey's *Heads Up Hockey* names *"the boards or goal post"* in the same sentence, and it *"can occur at walking speed"* — hit either with an arm or a leg, *"anything but your head first"*. That is every level and every book, not a checking-league point. **The drive ends at the paint, and at the red line around it** — the books split on whether that line counts as crease, so staying off it is the reading that holds either way. A skate in there risks the goal under every book, and under USA Hockey it can cost you the zone as well: **Rule 625(b)** takes the faceoff out to the nearest neutral-zone spot where an attacking player *"stands, holds their stick, or skates through the goal crease"* — though only while your team has possession with the puck in the attacking zone and the goalkeeper is in contact with his crease, and the *Casebook* does not stop play for a stick or skate put there momentarily. ⚠️ **The goaltender is the half of this that is not just a lost goal.** Contact you initiate with a goalie who is in his crease can cost you the goal, incidental or not (**NHL and IIHF Rule 69.3**); **69.2** assesses a penalty for deliberate contact *"whether or not the goalkeeper is inside or outside the goal crease, and whether or not a goal is scored"*; and the tier reaches an ejection under **USA Hockey 607(c)**, and automatically for charging him under **Hockey Canada 8.5(b)**, and under **CARHA 52(b)** while he is in his crease. [The wide entry](#the-wide-entry) sets out where each book puts the crease line and what each does to a player who hits the goaltender; [Offensive Zone Play](offensive_zone_play.md) covers the attacker in the crease.
...
3. You are in the zone with the linesman's arm up. What clears the delayed
   ...
   and which of the acts that bring the whistle is written in only three of
   the six rule books? *(§Delayed offside and tagging up — three consequences for
   the entry)*
...
1. **Carry it in where you can, and treat the dump as a tool rather than a default.** Carrying is worth about twice as much as dumping it in — 0.53–0.62 unblocked shot attempts per entry against 0.22–0.28, at 5-on-5, across Tulsky's full 330-game NHL sample from 2011-12. Those are attempts including misses, not shots on goal. That two-to-one shape has replicated across other trackers.
2. **So take the carry whenever you rate yourself better than about one in three to complete it — a failed carry costs far less than it feels.** About −0.08 net shots, cheaper than dumping and changing. Those come from Tulsky's Washington games alone and average every attempt rather than the marginal ones, so it is a probability argument and not a licence — one in three still means shooting it deep often. **And all of that cost is defensive:** a failed carry produces no shots of its own, and in Tulsky's own columns it gives up more shots on the opponent's next entry than any other entry type. Thibaud Chatel priced that same half in expected goals rather than shots, tracking a different league — his four scenarios barely separate, but the failed carry is the worst of them, 0.015 expected goals against to a failed dump's 0.013. **A separate test of the same objection, not an extension of Tulsky's number.**
3. **Drag that trail skate and attack the line at full speed — head up.** You are onside while one skate has not completely crossed, judged at the instant the puck completely crosses, so your body, hands and stick cross before your skate does — into the ice a defenceman standing up at their own line is defending, which is what the head is up for. **The NHL, the IIHF and the PWHL allow that trailing blade to be airborne; USA Hockey Rule 630(a) and Hockey Canada Rule 6.11 require it to be touching, and CARHA's book answers it neither way — so find out which book you are under.**
4. **On a delayed offside, hold at the line — then tag as one: every attacker in the zone puts a skate on the blue line, and the zone is empty before anyone re-enters.** That is the requirement in every book that writes the test; the wording differs and the requirement does not. Check your own book for one exception: under USA Hockey Rule 630(d) there is no tag-up at all below Youth 15-Only, Girls 16U, High School and Adult, where the whistle is immediate — a restriction Hockey Canada does not impose. **⚠️ And never close on the defending carrier expecting a whistle to arrive first.** Only the NHL, the IIHF and the PWHL stop play because you are *about* to hit him; USA Hockey 630(d), Hockey Canada 6.11(b)(ii) and CARHA 72(e)(2) write no contact trigger at all, so under those three nothing you can rely on stops play before you get there, and he is not braced for you. **A defenceman turned to retrieve has his back to you, so hitting him there is checking from behind** — and none of the three prices it cheaply: Hockey Canada attaches a game misconduct to every checking-from-behind minor (Rules 7.5(a) and 7.5(e)), CARHA 53(a) carries one on both of its options, and USA Hockey's *Casebook* takes the minor off a forceful check on a player standing along the boards. [Delayed offside and tagging up](#delayed-offside-and-tagging-up--three-consequences-for-the-entry) sets the wordings and the tiers out in full, and [Angle, don't chase](#angle-dont-chase) covers how to arrive on a retriever without becoming that hit.
5. **Attack the space between two defenders rather than at one of them — and go through that seam with your head up.** One defender has an easy job; two sharing a seam have a problem. **The middle seam is also the one route where both can stand you up, so head up is the condition of the drive, not an extra.** Under the IIHF book a player skating head up has an entitlement the rule states in terms — an opponent *"does not have the right to hit them to the head or neck"* — and it holds whether or not you have the puck (Rule 48.1). **⚠️ Put your head down and you give that away**, because the same rule declines to penalise a square check with no upward motion on a carrier skating head down **toward the checker**. How far that carve-out reaches is unresolved in the IIHF's own text, and **both halves are the IIHF's own — the NHL writes neither.** [Three things that do not bend](#three-things-that-do-not-bend-whichever-pattern-you-run) sets out its limits, and [Rules Primer](../foundation/rules_primer.md) compares what the other books do instead.
6. **The puck carrier should be the last to the line and the support first — waiting at it, onside, not across it.** That ordering is a coaching convention rather than a rule, and arriving with speed is not the same as arriving early: **be slower thirty feet out so your acceleration peaks as the puck crosses the line.** Crossing it and then having to slow down to stay with the play means you were early.
7. **Alone at the line, turn away instead of forcing it — protect the puck, look back up the ice, and re-attack a second later with support.** The delay runs against instinct, because turning away from the offensive zone feels like retreating. **One safety override outranks the play: curl in open ice, not against the wall.** Never take contact with your back to the boards. Near the wall, get your skates parallel to it, forearm and hip, head up and chin off your chest — and not on the point of your shoulder, which USA Hockey rules out in terms, and not on the whole shoulder if you can help it, which is how USA Hockey hedges its broader instruction ([Body Contact and Battles](../technique/body_contact_and_battles.md), which covers this).
8. **Pick the dump to the read in front of you, and never release one without a chaser already going there.** Hard rim, soft area dump, chip past a committing defenceman, cross-corner, flip over a trap — each answers a different picture. **Land a flip short of the goal line, because an overshoot is icing** — and shorthanded that depends on your book: the exemption is withheld below USA Hockey's Youth 15-Only and Girls 16U (Rule 624(b)(1)) and withdrawn by the NHL only where a major expires with nobody on the penalty bench to exit, and then only until the next stoppage (Rule 81.6), while the IIHF book Britain plays carries neither limit — **though its Rule 20.3 wants a substitute in the penalty box immediately when a major empties it, and Rule 68.2 makes failing that a bench minor.**
9. **Dump to the corners, not straight behind the net, wherever the trapezoid applies** — the NHL and IIHF, and reportedly the KHL, though no KHL rulebook was available to check that one. Neither the USA Hockey nor the Hockey Canada book has one, so a league running on either has none. In England and Wales, England Ice Hockey's *Rules & Regulations* 22.3 say it is not currently enforced, so plan for their goalie to come out, and ask your league.
10. **Angle to take away the retriever's easiest escape rather than to shorten the distance.** Angling is taking the escape away, not arriving through him: a defenceman turned to retrieve has his back to you, and hitting him there is checking from behind. **And their goalie is not "fair game" outside his crease in any book here** — every one of them prints that phrase (IIHF and NHL 42.1, USA Hockey 607(d), Hockey Canada 8.5, CARHA 52(b), PWHL 42.1) — Hockey Canada 8.5(b) makes charging the goaltender an automatic major plus game misconduct, and USA Hockey penalises even accidental contact (Note 1 to 607(d)) where the IIHF and NHL allow incidental contact only while he is playing the puck and only on a reasonable effort to avoid it. Whether you stand up at your own line, receive the entry, or squash-and-slide is a system your coach chooses, not a law.
11. **Drive to the edge of the blue paint and stop there — clear of the red line around it, short of the goaltender, and head up.** **Never meet a goal post head first, and never duck**: a head-down, head-first hit on a post is the same cervical-spine injury as one on the boards, and it can happen at walking speed. **That is every level and every book, not a checking-league point.** Screening him from outside the crease, without contact, is normally legal and worth doing; it is a skate inside the paint, or on the red line at its edge, that risks the goal. The books answer that line in two directions — and one of them, the IIHF, answers it in two directions inside its own covers — so staying off it is the reading that holds either way. ⚠️ **Losing the goal is the smaller half of the cost.** Contact you initiate with a goalie who is **in** his crease can cost you the goal whether or not it was incidental (**NHL and IIHF Rule 69.3**), the penalty under **69.2** runs separately from the goal — it is assessed *"whether or not a goal is scored"* — and the tier reaches an **ejection** under **USA Hockey 607(c)** — with no referee's discretion in it for **charging** him under **Hockey Canada 8.5(b)**, and under **CARHA 52(b)** while he is in his crease. ⚠️ **And the crease is not where this stops reaching you:** once he has left it to play the puck, **NHL and IIHF Rule 69.4** penalises an attacker whose *deliberate actions* prevent his return — a foul that does not wait for you to touch him, because the clause turns on intent, not contact. [The wide entry](#the-wide-entry) sets out where each book puts the crease line, what the British reader's book does with that exception, and what each book does to a player who hits the goaltender; [Offensive Zone Play](offensive_zone_play.md) covers the attacker in the crease.

---

*Sources — retrieved 27 July 2026:*

*Rules: [NHL Official Rules 2025-2026 (PDF)](https://media.d3.nhle.com/image/private/t_document/prd/slwjuaqwmuvj5bkplixo.pdf) — Rule 83 (Off-side), Rule 1.8 (goalkeeper's restricted area — the painted dimensions: eight feet from each post at the goal line, eleven feet at the boards, a 28-foot span), Rules 27.8 and 63.2(viii) (the restricted-area penalty — cited for the offence only; its six-foot figure is stale), Rules 1.5–1.7 (lines, division of ice, crease), Rule 81.6 (the shorthanded icing exemption; its second paragraph, which judges icing on an expiring penalty *"at the instant the penalty expires"* and holds that *"[s]hould the puck be released from the stick of the player shooting the puck down the ice prior to the penalty expiring, the icing infraction shall not apply"*; and its third, withdrawing the exemption from a team short by a major that has *"neglected to ensure there is a player on the penalty bench to exit upon the expiry of the penalty"*, read to its last sentence for how long that lasts — *"[t]hey may substitute for this penalized player at the next stoppage of play"*), **Rule 20.3**, read 14 September 2026 for when that empty bench actually bites — a team whose penalised player has been removed or injured *"does not have to place a substitute player on the penalty bench immediately, but must do so at a stoppage of play prior to the expiration of the major penalty"*; and where that player has *"minor penalties in addition to the major penalty"* running on the clock alongside it, the same rule requires the team to *"place a substitute on the penalty bench immediately"*, read 14 September 2026, and only *"[f]ailure to place a player on the penalty bench prior to the expiration of the major penalty"* costs them the exemption, which the same sentence then ends — the team continues *"one player short (but not officially considered short-handed) until the next stoppage of play"*, **Rule 68.2**, read 15 September 2026 for what the empty bench costs before the icing does — the penalised team *"shall place a substitute player on the penalty bench before the major penalty expires"* and *"[a]ny violation of this provision shall be treated as an illegal substitution under this rule calling for a bench minor penalty"*, and **Rule 42.1 Charging**, read 3 September 2026 for the goalkeeper chased outside the crease: *"A goalkeeper is not 'fair game' just because he is outside the goal crease area. The appropriate penalty should be assessed in every case where an opposing player makes unnecessary contact with a goalkeeper. However, incidental contact, at the discretion of the Referee, will be permitted when the goalkeeper is in the act of playing the puck outside his goal crease provided the attacking player has made a reasonable effort to avoid such contact."* · [IIHF Official Rule Book 2025/26 (PDF)](https://blob.iihf.com/iihf-media/iihfmvc/media/contentimages/3_the_iihf/2025-26_iihf_rulebook_22122025-v1.pdf) — **the rule set British hockey plays under**, read directly for this document: Rule 83.1 (off-side — the both-skates test, the one-skate on-side test and the "plane" of the blue line, all word-for-word with NHL 83.1; **no** blade-only definition of "skate" in this edition, and a puck-carrier carve-out asking only for possession and control *"prior to their skates crossing the leading edge"* — both of which changed in the 2026/27 book, next), Rule 83.3 (delayed off-side and tag-up, including *"skate contact with the blue line, '3D' judgement is not used in this situation"*), Rule 83.4 (disallowed goal, word-for-word with the NHL including the own-goal exception), Rule 83.6 (face-off locations), Rule 27.7 (goalkeeper restricted area), **Rule 81.6** (read 14 September 2026, word for word the same in the 2025/26 and 2026/27 editions: two paragraphs only — the exemption and the expiry-timing test — with **neither** of the NHL's and USA Hockey's carve-outs, no major-penalty paragraph and no classification limit), **Rules 20.3 and 68.2**, read 15 September 2026 and word for word in both editions — 20.3 requires that a team whose major-penalised player *"has been removed from the game or is injured"* *"must place a substitute in the penalty box immediately"*, and 68.2 closes with the NHL's own sentence, *"[a]ny violation of this provision shall be treated as an illegal substitution under this rule calling for a bench minor penalty"*, and **Rule 15.1**, read 15 September 2026 for why that bench minor is a contingency rather than a certainty — an infraction by the team *not* in control of the puck draws only a raised arm, with play running on until *"the team to be penalized gains control of the puck"*, so a major can expire before the whistle arrives, Rule 48.1 (the head-down carve-out, read with its opening sentence and with the scope caveat Rules Primer covers) Rule 43.2 (checking from behind, no minor option), and **Rules 42.1 and 69.4**, read 3 September 2026 and re-read 14 September 2026 — each carrying its own version of the *"A goalkeeper is not 'fair game'"* sentence, and the two are not word for word: 42.1 reads *"outside the goal crease **area**"* and reaches *"an **opposing** Player"*, where 69.4 reads *"outside the goal crease"* and reaches *"an **attacking** Player"*, and 42.1 carrying the permission with its limit inside the same sentence: *"However, incidental contact, at the discretion of the Referee, will be permitted when the goalkeeper is in the act of playing the puck outside their goal crease, provided the attacking Player has made a reasonable effort to avoid such contact"* · [IIHF Official Rulebook 2026/27, Version 1.0, June 2026 (PDF)](https://blob.iihf.com/iihf-media/iihfmvc/media/downloads/rule%20book/2026-27_iihf_rule_book.pdf) — **the IIHF's current edition. Every IIHF rule this document cites — 83.1, 83.3, 83.4, 83.5, 83.6, 27.7, 43.2 and 48.1 — was compared against it word by word on 1 September 2026, and only 83.1 changed.** The differences the comparison threw up in the other seven are hyphenation and page furniture in the extraction, not rule text. **The IIHF *Situation Handbook* 2026/27 is a separate book and was not compared**, so the Situation 83.34 and 83.35 rulings cited above are the 2025/26 Handbook's. Rule 83.1 changes in two places, both moving the IIHF's text onto the NHL's: it inserts *"For the purposes of this rule, a 'skate' is to be considered the blade of the skate only"* into the on-side test, and it rewrites the puck-carrier carve-out to require *"'possession and control' of the puck with their stick prior to both skates completely crossing the leading edge"*. The both-skates test, the one-skate on-side test and the "plane" sentence are unchanged. ⚠️ **Which edition a British reader plays under depends on the competition, and for one of them it is published.** The **Motorpoint EIHL Casebook 26-27** Preamble provides that *"For all rules not mentioned in the EIHL Casebook, the official 2026 - 2027 IIHF Rulebook and IIHF Situation Handbook will serve as the official ruling"*, and the Casebook was searched flattened for Rule 83, off-side and icing and mentions none of them — so an **Elite League** game runs the 2026/27 book's Rule 83. ⚠️ **That Casebook carries three disagreeing version stamps** — page footers reading Version 1.0 dated 1 August 2026, a front-matter version table reading Version 1.1 dated 8 September 2026, and PDF metadata reading Version 1.0 with the September date — and its Preamble provides that *"Each rule update will be sent by email, on the EIHL web site and in print"*, so the copy read here may already have been added to. **Below the Elite League it remains unresolved**: the In-House Rules adopt the 2025/26 book by version number while their cover names the current one, and they amend Rule 83 in neither; [UK and England Rules](../foundation/uk_rules.md) sets that out · [IIHF Situation Handbook 2025/26 (Version 1.1, August 2025)](https://blob.iihf.com/iihf-media/iihfmvc/media/downloads/officiating%20files/situation%20handbook/2025_iihf_situationhandbook_17082025-v1_1.pdf) — **Situation 83.34**, the airborne clearing case: A31 jumps about a metre before the blue line and lands on it, and *"A31 is considered to have cleared the attacking zone the instant the skate makes contact with the blue line. Rule 83.3 (I). Should another Team A player enter the attacking zone while A31 is airborne, the delayed off-side shall remain in effect."* Situation 83.35 rules the same for a player sliding feet first, and adds that the skate *"must touch the ice, plane does not apply here"*. Its Foreword places the Handbook below the Rulebook: *"in case of any contradictions, the valid IIHF Rulebook is always the basis."* **Situation 83.29** is the airborne *entry* case and is quoted above: *"As the puck crosses the attacking blue line, an attacking player jumps into the air right at the blue line. Is this off-side?"*, answered *"The skates of the player jumping must be on its own side of the leading edge of the blue line and have yet to break the 'plane of the leading edge' prior to the puck crossing the leading edge. Rule 83.1."* ⚠️ **Its answer sits on the far side of a page break from its question, so a short extraction returns the question alone.** Read against the [IIHF Situation Handbook 2026/27 (PDF)](https://blob.iihf.com/iihf-media/iihfmvc/media/downloads/officiating%20files/situation%20handbook/2026-27_iihf_situation_handbook.pdf) on 23 September 2026 and word-identical there. All three Situations are word-for-word identical in the earlier Version 1.0, June 2025 · [IHUK In-House Rules 2026-27, Revision 1 (published 25 July 2026)](https://englandicehockey.com/wp-content/uploads/2026/08/IHUK-In-House-Rules-2026-2027.pdf) — **the current British document**, read 31 August 2026: the Introduction, which adopts the IIHF book *"in England, Wales, Scotland and Northern Ireland (the Home Countries)"*, and **Rules 37 and 38** — *"on-ice video review is not available in EIH or SIHA competition"* and therefore *"a Coach's Challenge is not permitted in EIH or SIHA competition"*, which removes IIHF Rule 83.5 in domestic play. ⚠️ **Both are still written to the narrower EIH-or-SIHA wording in this edition**, where the document as a whole now reaches all four Home Countries; [UK and England Rules](../foundation/uk_rules.md) sets out what that leaves open. Searched flattened: the In-House Rules make **no** amendment to IIHF Rule 83 (Off-side), Rule 81 (Icing) or Rule 27, in this edition or the last · [IHUK In-House Rules 2025-26, Revision 1](https://englandicehockey.com/wp-content/uploads/2025/11/IHUK-In-House-Rules-25-26-R1.pdf) — **superseded, cited only for the comparison**: it carried Rules 37 and 38 in the same words, but applied *"in England and Scotland"* alone · [Scouting The Refs — NHL Rule Book Updates for 2025-26](https://scoutingtherefs.com/2025/09/49364/nhl-rule-book-updates-for-2025-26/) · [England Ice Hockey Rules & Regulations 2024-2025](https://englandicehockey.com/wp-content/uploads/2024/09/1.0-5.0-EIH-Rules-and-Regulations.pdf) — Rule 22.3, the Goalkeepers' Restricted Area "not currently enforced" · [USA Hockey Official Playing Rules 2025-29 (PDF)](https://cdn2.sportngin.com/attachments/document/945a-3442848/2025-29_USAH_Playing_Rules.pdf) — **Rules 101–109** (Section 1, *The Rink*) and **Rule 610 Delaying the Game**, read for a goalkeeper's restricted area: the book marks none and places no limit on where a goalkeeper may play the puck; **Rule 614(c)**, which does restrict where a goalkeeper may *freeze* it, including outside the *"goalkeeper's privileged"* area defined in its own Note, which excludes the corners; Rule 624(b)(1) (the shorthanded icing exemption, the classification list it is confined to, and its own timing test — *"The determination is made at the time the penalty expires and if the puck was shot prior to the penalty time expiration, regardless as to the position of the penalized player, no icing shall be called"*), Rule 630(a) (the skate-contact test for off-side, and its Note that one skate touching the line is enough) and Rule 630(d) (delayed off-side and tag-up, its classification restriction, 630(d)(2) on clearing "simultaneously… at the same instant", and the two conditions that turn a delayed off-side into a whistle — an attacking player touching the puck, or attempting to gain possession "or continues to apply pressure to the defending puck carrier", there being no about-to-make-contact condition), and Rule 617 (Goals and Assists — 617(b)(2), a goal is legally scored if it "enters the goal as a result of any action by the defending team", and 617(c)(8), a goal is not allowed where "The puck enters the attacking zone illegally"). Rule 630 carries no disallowed-goal provision for a shot that creates a delayed off-side, and the Playing Rules nowhere resolve that case between 617(b)(2) and 617(c)(8). Also **Rule 608 Checking from Behind**, read for the forechecker closing on a defenceman who has turned to retrieve — its Note defines the foul as a check delivered *"directly from behind, or diagonally from behind"* and says *"This includes body checking or pushing an opponent from behind in open ice or directly into the boards or goal frame"*; 608(a) is a minor plus a misconduct or a major plus a game misconduct, and 608(b) a major plus a game misconduct where the check *"recklessly endangers an opponent, or causes them to go head first into the boards or goal frame"*. Also **Rule 607 Charging**, read 3 September 2026 — 607(c), *"A minor plus a misconduct or a major plus a game misconduct penalty shall be assessed to a player who body checks or charges a goalkeeper while the goalkeeper is within the goal crease or privileged area"*; 607(d), the *"NOT 'fair game'"* sentence; and its **Note 1**, which writes no incidental-contact permission at all — *"For the purpose of this rule, any accidental or unavoidable contact that occurs with the goalkeeper shall be penalized under the Interference rule. Any deliberate body contact or check that is delivered to the goalkeeper shall be penalized as charging"* — together with Note 2's definition of the privileged area · [USA Hockey Official Rules and Casebook 2025-29 (PDF)](https://cdn3.sportngin.com/attachments/document/4f1a-3442850/2025-29_USAH_Playing_Rules___Casebook.pdf) — **Casebook Rule 630, Situation 7**, the airborne trail skate on an entry, read 23 September 2026 — an attacker whose *"trailing skate is directly above the ice in the Neutral Zone (off the ice)"* is offside, because *"they must have at least one skate in contact with the ice in the Neutral Zone, which in this instance includes the blue line, at the instant the puck completely crosses the blue line"* (rule reference 630(a Note)); **Casebook Rule 630, Situation 8**, for the definition of the word, read the same day — *"The word 'skate' in the Rule refers to the blade or the boot of the skate"*, which is the opposite of the NHL's and the PWHL's blade-only definition and runs in the player's favour; **Casebook Rule 630, Situation 41**, which does resolve it: no goal during a delayed off-side, *"[e]ven if the defending team shoots, passes or deflects the puck into its own goal"*, and not even where the attacking team cleared the zone first. **Read to the end of it: the same situation closes by allowing play to continue, and any goal legally scored to stand, where the puck was not shot directly on goal and the attacking team has cleared the zone** — the bar is the offending puck and the live delayed off-side, not everything that follows a tag-up. That closing sentence is printed with a word dropped from it and is paraphrased rather than quoted throughout this document. Situation 41 gives its rule reference as 630(d.3), a sub-clause the printed Rule 630(d) does not contain; **Casebook Rule 630, Situation 38** (rule reference 630(d.2)), which carries the forcing-deeper trigger the Playing Rules do not word — a defender may carry the puck anywhere in his own zone "as long as they are not being forced deeper into their zone by an attacking player", and the Linesperson "should be alert to stop play as soon as the defending puck carrier is forced back further into their Defending Zone or as soon as contact, however slight, is made with the puck or puck carrier"; **Casebook Rule 630, Situation 39**, whose general rule is that an offside player who declines to tag up and instead pursues the puck should have play stopped immediately, because "[t]he potential for unnecessary body contact should not be allowed in a situation where an infraction (offside) has already occurred" — a rationale attached to pursuit of the puck, not a written imminent-contact trigger; **Casebook Rule 607, Situation 2**, the head-down charging answer above; and **Casebook Rule 608, Situation 1**, read 31 August 2026, which sets the criteria for the checking-from-behind tiers — the minor plus misconduct *"must be assessed"* only for a push on an opponent *"skating ahead of them and not near the boards"* or *"minimal body contact"* making *"this 'pinch' against the boards"*, while the major plus game misconduct *"or match penalty, must be called"* where a player *"forcefully checks an opponent who is standing along the boards (back toward the middle of the ice)"*, where a player is *"thrust head first into the boards or goal frame"*, and where a check from behind *"recklessly endangers the opponent ... regardless of whether or not board contact is made"*, with the *"danger zone"* given as *"approximately ten feet out from the boards"* · [Hockey Canada Playing Rules 2026-2028 (PDF)](https://cdn.hockeycanada.ca/hockey-canada/Hockey-Programs/Officiating/Downloads/2026-28-hc-rulebook-e.pdf) — **Section 1, Rules 1.1–1.14** and **Rule 10.1 Delay of Game**, read for a goalkeeper's restricted area: the book marks none and 10.1 places no limit on where a goaltender may play the puck; Rule 6.11 (off-side, and the sentence restricting the test to skates "in physical contact with the ice surface"), Interpretation 3 to Rule 6.11(a) (the airborne trail skate, ruled OFF-SIDE), Rule 6.11(b) (delayed off-side and tag-up). ⚠️ **Rule 6.11(b) carries two separate i./ii. lists**, so "6.11(b)(ii)" names two different rules: the **tag-up clause** sits under "the Linesperson will drop their arm to nullify the off-side violation and allow play to continue if", and the **stop-play clause** sits under "During a delayed offside, if". The **tag-up clause** requires skate contact from every attacker, "so that the attacking zone is completely clear of attacking players", with no simultaneity requirement and no classification restriction; the **stop-play clause** reaches only touching the puck and attempting to gain possession of a loose puck "while the puck is still in the attacking zone", with no pressure limb and no contact limb. Also 6.11(b)'s disallowed-goal provision, "the goal will not be allowed as the original shot was off-side", and Note 1 extending that to a shot that goes in after the zone has been cleared. No own-goal exception appears in Rule 6.11 or in any of its thirteen interpretations. Also Rule 6.11(d) (intentional off-side — including a player who deliberately makes a play on the puck "or against an opposing player" while knowing they are off-side, which 6.11(d) costs at a face-off "at an end zone face-off spot, in the defending zone of the offending team" rather than at a penalty), and Rule 8.7 Clipping, whose preamble provides that "Players may not crouch down to avoid being bodychecked" and which states no division or classification scope for that prohibition. Rule 8.7 is Hockey Canada's clipping rule; the penalty for a checker who hits a ducking opponent is a separate provision, set out with its scope in [Rules Primer](../foundation/rules_primer.md). Also **Rule 7.5 Checking from Behind**, read for the forechecker closing on a defenceman who has turned to retrieve — its preamble directs referees to be *"aware of the tragic consequences of spinal injuries"* and lists, among the situations to enforce strictly, *"Where a player along the boards, with their head down looking for the puck or 'digging the puck out,' is hit from behind and driven head-first into the boards"*; 7.5(a) is a Minor plus a Game Misconduct for an intentional hit from behind *"anywhere on the ice"*, and 7.5(b) carries two limbs — a Major plus Game Misconduct *"at the discretion of the Referee, based on the degree of violence of the impact"* with nobody injured, and the same pair that *"must be assessed"* where such a foul injures the opponent. Also **Rule 8.5 Interference with the Goaltender**, read 3 September 2026 for the dump-in chase — its *"A Goaltender is not 'fair game' just because they are outside their goal crease"* paragraph; **8.5(b)**, whose first limb is discretionary (*"A Major penalty and Game Misconduct penalty, at the discretion of the referee, based on the degree of violence of impact, may be assessed to any player who commits interference with the goaltender"*) and whose second is not (*"A Major penalty and Game Misconduct penalty will be assessed to any player who charges the goaltender"*); **8.5(e)**, *"A Game Misconduct penalty must be assessed any time a Major penalty is assessed for interference with the goaltender"*; and **Interpretation 3 to Rule 7.4(b)**, *"Where a goaltender is contacted by an attacking player in their crease and that action could be described as charging, a Major penalty and Game Misconduct penalty for Interference must be assessed"*. ⚠️ **8.5(b)'s own Note 1 cites "Rule 7.4(b)(Interpretation 2)" for the definition of charging; the interpretation attached to 7.4(b) is numbered 3, and the one numbered 2 is attached to 7.4(a) and defines a blindside hit.** The definition of charging used here is **Rule 7.4**'s own list, which reaches a player who "[b]uilds up speed by taking two or more strides immediately prior to making contact" · [PWHL Official Rule Book 2025-2026 (PDF)](https://assets.contentstack.io/v3/assets/bltebdb4296e05d53db/bltb8440fe08a5e6dc5/69178cd0a582d7db321ad107/2025-2026_PWHL_Rulebook.pdf) — **Rule 85, not Rule 83: the PWHL renumbers off-side.** Read 23 September 2026 for **85.1**, which is the NHL's off-side rule in the feminine and carries both the "plane" test and the blade-only sentence — *"On her own side of the line shall be defined by a 'plane' of the blue line ... For the purposes of this rule, a 'skate' is to be considered the blade of the skate only"* — and for **85.3**, whose clause (i) nullifies the delayed off-side when *"[a]ll players of the offending Team clear the zone at the same instant (skate contact with the blue line)"* and which carries all four of the NHL's stop-play triggers, the about-to-make-contact one included · [CARHA Hockey Official Rule Book, printed 2020 (PDF)](https://carhahockey.ca/wp-content/uploads/2024/01/RuleBk-2020-interactive-1.pdf) — **Rule 72**, read 23 September 2026. Its 72(c) Note 1 keeps a player onside where either skate is *"in physical contact with, or on their own side of, the blue line"*, ⚠️ **and the book never defines "own side" anywhere** — it prints no "plane" sentence, no skate-in-contact-with-the-ice test and no interpretation on the point, and no separate CARHA casebook is held here — the book’s own *CARHA HOCKEY – CASE SITUATIONS* section prints a single situation under Rule 72, and it asks a different question, whether a player propelling the puck may skate backwards over the blue line (*"RULING: YES, providing the player has “Possession and Control” of the puck"*), which Rule 72(a) Note 2 already answers in the rule text — so it answers the airborne trail skate neither way rather than answering it like the NHL. Its 72(e)(2) nullifies a delayed off-side when *"all attacking players in the attacking zone (at the time the puck crosses the blue line) clear the attacking zone by making skate contact with the blue line"*, without naming an instant, and its same clause stops play if *"any attacking player touches the puck or attempts to gain possession of a loose puck while the puck is still in the attacking zone"*. **Governs CARHA-affiliated leagues only**, and is not a general rec-hockey standard*

*Analytics: [Tulsky, Detweiler, Spencer & Sznajder, "Using Zone Entry Data To Separate Offensive, Neutral, And Defensive Zone Performance", MIT Sloan Sports Analytics Conference 2013 (PDF)](http://hockeyanalytics.com/Research_files/Using%20Zone%20Entry%20Data%20To%20Separate%20Offensive,%20Neutral,%20And%20Defensive%20Zone%20Performance.pdf) — refetched 14 September 2026, HTTP 200, 700,520 bytes, no redirect, and **Table 3 read in full**, including the two working columns the table above compresses away: shots for and shots against on the opponent's next entry, where a failed carry's 0.40 against is the largest figure in the column · [Notes From The Rafters — "Proving Tulsky Right: Zone Entries" (PWHPA / Sportlogiq data)](https://notesfromtherafters.substack.com/p/proving-tulsky-right-zone-entries) · [Thibaud Chatel — "Assessing the Risk Management of Zone Entries", 27 April 2022](https://thibaudchatel.substack.com/p/assessing-the-risk-management-of) — **the primary for the 47% / 18% pair and for the xG figures**; refetched 14 September 2026, HTTP 200, 186,225 bytes, no redirect, and every string quoted above located in the rendered body with HTML comments and scripts stripped · [Hockey's Arsenal — "Enter the Offensive Zone With More Offence and Less Risk" (restating Sznajder and Chatel)](https://hockeysarsenal.substack.com/p/enter-the-offensive-zone-with-more) — where this document first met both pairs of figures, and still the only route here to the Sznajder pair; refetched 14 September 2026, HTTP 200, 171,686 bytes · [Sports Illustrated — "Fancystats quietly leading NHL teams to dump the dump-and-chase strategy" (2014)](https://www.si.com/nhl/2014/01/15/fancystats-quietly-leading-nhl-teams-to-dump-the-dump-and-chase-strategy)*
...
*The crease line (the §The wide entry, §The middle drive and §Entering with numbers passages, Common Mistakes and Key Takeaway 11): read in the primary texts on 18 September 2026 and quoted from them. **IIHF** Rule 1.7 — *"In front of each goal frame, one goal crease shall be marked by a red line. The marked line belongs to the Goal Crease."* **USA Hockey**, the Note to Rule 625(b) — *"The goal crease area shall include all the space outlined by the semi-circular crease lines (including crease lines) and extending vertically to the level of the top of the goal frame"* — with the *Official Rules and Casebook* answering the question directly at **Rule 103, Situation 4**: *"Is the two inch red line outlining the goal crease considered to be a part of the goal crease? Yes. Rule Reference 103(b)."* **Hockey Canada**, its Glossary entry for GOAL CREASE — *"The lines of the crease are considered part of the crease."* ⚠️ **The NHL writes no such sentence.** Its Rule 1.7 says only that the goal crease area *"shall include all the space outlined by the crease lines"*, and its **Reference Table 14, Situation 5 E** allows the goal where an attacking player *"plants himself on the crease line or outside the goal crease, as to obstruct the goalkeeper's vision"*, as does **IIHF Appendix IV, Table 16, Situation 5 E** in the 2025/26 edition, renumbered Table 14 in 2026/27 — so the IIHF book answers the same play in two directions. ⚠️ **Only the IIHF answers this question in rule text.** USA Hockey answers it in the **Note** to 625(b) and in a *Casebook* situation, and **Hockey Canada in its Glossary** — its own **Rule 1.5(c)** uses the NHL's looser formula, *"the space outlined by the crease lines"*, so that book too can be read both ways. The instruction to stay off the line therefore rests on one book's rule text, one book's Note and Casebook, and one book's Glossary, set against two reference tables — and **nothing in any of the four books ranks those layers against one another.** That is what makes the conservative reading the safe one rather than merely the cautious one. [Rink Map](../foundation/rink_map.md) owns this split and states it the same way.*

*Contact with the goaltender on a net drive (the same three passages, Common Mistakes and Key Takeaway 11): read in the primary texts on 18 September 2026. **NHL Rule 69.3** — *"If an attacking player initiates contact with a goalkeeper, incidental or otherwise, while the goalkeeper is in his goal crease, and a goal is scored, the goal will be disallowed"* — with **69.4** for the goalkeeper outside his crease and **69.2** for the penalty, which attaches *"whether or not a goal is scored"*. **The IIHF prints all three at the same numbers**, its 69.3 reading *"a relevant contact"* where the NHL writes *"contact"*. **USA Hockey Rule 607(c)** for the tier, and **Hockey Canada Rule 8.5(b)** for the automatic major plus game misconduct on charging the goaltender — the same two provisions §7 of this document quotes for the dump-in chase, which is a different play under the same rules. **CARHA Rules 52, 66 and 30 were read on 24 September 2026** and quoted from the extraction on disk: **52(b)** for the mandatory major plus game misconduct *"to any player who charges a goalkeeper while the goalkeeper is within the goal crease or who injures an opponent as a result of a charge"*, **66(b)** for deliberate contact *"whether in or out of the goal crease"* and **66(e)** for the mandatory major on injuring an opponent by interference, and **30(a)** for the ejection every CARHA major carries.*

*Boards and goal-post safety (the override on the delay, and the head-up limb on the net drive — [Body Contact and Battles](../technique/body_contact_and_battles.md) carries the full treatment): [USA Hockey — Heads Up Hockey Program Guide (PDF)](https://cdn4.sportngin.com/attachments/document/0138/6193/HUH_Program_Guide_FINAL.pdf) — Session 5, "Giving A Check" — under its "Checking from behind" subheading, which is not the session's title — for *"Even a light hit from behind could inflict a severe head or neck injury"*; and "How spinal injuries happen in hockey" for *"it can occur at walking speed"*, which the guide attaches to *"the head being slightly flexed (head down) while making head-on contact with the boards or goal post"* and not to a player turning their back. **"This" in that sentence is the spinal injury, not the collision**, which is why the override renders it as the injury occurring at walking speed. **The finding is a restatement:** USA Hockey credits *"research done among a wide range of hockey players"* and prints the citation beneath it — **Charles H. Tator et al., *Spinal Injuries Due To Hockey*, Canadian Journal of Neurological Sciences 11:34–41** — not read in the original here, only the USA Hockey page reproducing it · [USA Hockey — Introduction To Body Contact (PDF)](https://cdn4.sportngin.com/attachments/document/55c1-2114751/IntroductiontoBodyContact.pdf) — §5 "Checking From Behind", carrying the same light-hit sentence, and item 1 "Angling In" of its *Areas/Drills to Cover When Teaching: "Heads Up" Hockey* list, for *"Never hit the boards or glass with the tip of your shoulder – it can cause injury"*, which the Program Guide carries too, differing only in its dash; and its *Basic Principles of Heads Up Hockey*, for *"Hit the boards or goal posts with an arm, a leg, or anything but your head first."* **That longer sentence is this manual's alone** — the Program Guide prints only *"Hit the boards with anything but your head first"*, without the goal post. **The manual is built on the 2007-09 rule book, so any rule it states may since have been superseded; the sentences quoted from it here are its technique teaching, not rule citations.** **The two broader shoulder sentences are from the same manual:** its §B teaching points for a player *"checked while skating parallel to the boards"* for *"[a]void hitting the boards with the shoulder"*, and its taking-a-hit demonstration list for *"No shoulder contact on the boards, if possible"* — a sentence the Program Guide prints in the same list word for word. Both refetched and the quoted strings located in the extracted text on 31 July 2026; the shoulder sentence located in both extractions on 9 September 2026; the two broader sentences located on 19 September 2026 in the extractions on disk rather than by a refetch.*

*Limitations worth knowing: Tulsky's paper carries two different scopes and this document keeps them apart. The shots and goals per carried-in and dumped-in entry (Table 1: 0.53–0.62 against 0.22–0.28) are the full 330-game sample. Everything that involves a **failed** attempt, a dump-and-change or a net value across two entries (Table 3: 0.55, 0.48, 0.24, −0.08, +0.42, +0.12, the 14% turnover rate, and the 34/26/44% thresholds derived from them) comes from the **Washington games alone**, because those were the games in which unsuccessful attempts and dump-and-change plays were recorded — the paper labels that table "as measured in Capitals games". The paper's own strategic conclusion runs toward carrying more, not less, and concerns NHL players only: "in general NHL teams probably could benefit from being more aggressive at the blue line." The authors also state that their averages are "still not sufficient data to say conclusively that teams should be trying harder to carry the puck in at every opportunity". The season the PWHPA / Sportlogiq figures cover is not stated in the source, which was published in July 2022 and refers only to "this last year's PWHPA Secret Dream Gap Tour" — 2021-22 is an inference, and is flagged as such in Section 1. The Chatel (47% / 18%) figures were taken from Hockey's Arsenal when this document was first written and are now cited from Chatel's own post, which carries the sample size and the population that the second-hand version drops; the xG figures are from that post too. **The Sznajder (0.66 / 0.29) pair is still second-hand** — Hockey's Arsenal and Chatel each restate it with different provenance (tracking dated 2014-16; a RITSAC 2018 presentation) and no publication by Sznajder himself was located here — his own Substack, All Three Zones, was searched on 14 September 2026 (HTTP 200) and neither figure appears on it, and the RITSAC conference domain did not resolve, so the presentation itself was not read. **Neither tracking data set is published**, so treat both as indicative rather than exact. Chatel's separate 2020 Hockey Graphs work, a different sample from the one quoted here, is cited in [Risk Management](../hockey-iq/risk_management.md). The NHL rulebook's own two descriptions of the trapezoid's goal-line dimension disagree (Rule 27.8 vs Rule 1.8), as noted in Section 7. The KHL is named above as a trapezoid league on general report only: no KHL rulebook was available to check it against, and the claim is flagged as unverified everywhere it appears.*
