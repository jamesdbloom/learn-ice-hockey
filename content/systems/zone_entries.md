...
Never: Turn your back to the boards going down the wall — if you are tight to it and cannot get off it, take it with skates parallel to the wall, forearm and hip, head up and chin off your chest
...
Rule: The other two books are not scoped to the crease either — USA Hockey 607(c) reaches a check or charge on a goalkeeper "within the goal crease or privileged area", and Hockey Canada 8.5(b) makes a major plus a game misconduct automatic for charging the goaltender, keyed to no area of ice
```
...
Middle drives only work if they are **committed**. A half-hearted middle drive is a player standing in the slot, which frees the defence rather than freezing it. **Committed still stops at the edge of the blue paint, and off the red line around it** — a skate inside the paint, or on that red line, is what risks the goal. ⚠️ **And a committed drive is not licence to hit the goaltender:** contact you initiate with a goalie who is in his crease voids the goal whether or not it was incidental (**NHL and IIHF Rule 69.3**), and all four books price a penalty separately from the goal — **NHL and IIHF 69.2** reach deliberate contact *"whether or not the goalkeeper is inside or outside the goal crease"*, **USA Hockey 607(c)** runs a check or charge up to an ejection while its 607(d) asks for a penalty *"in every case where an opposing player makes unnecessary contact with a goalkeeper"*, and **Hockey Canada 8.5(b)** makes the ejection automatic for charging. The wide entry above sets out how the four books split on the line itself, and quotes what each of them does to a player who hits the goaltender.
...
Rule: USA Hockey 607(c) reaches a check or charge on a goalkeeper "within the goal crease or privileged area" — "a minor plus a misconduct or a major plus a game misconduct" — and Hockey Canada 8.5(b) makes that automatic for charging the goaltender, keyed to no area of ice
```
...
Read: Those clauses are about a retrieval, not a save — a cover made after a save outside the crease is a different case in both books, where the carve-outs run the other way; Goaltender sets them out
...
- **Overshooting a flip dump — and then misjudging whether the kill is covered.** A puck crossing the goal line untouched from behind the centre red line is icing, and the shorthanded exemption is not universal: USA Hockey withholds it below Youth 15-Only and Girls 16U (Rule 624(b)(1)), and the NHL withdraws it where a major **expires** with nobody on the penalty bench to exit (Rule 81.6) — **not for the rest of the major: that ban lifts at the next stoppage** (Rules 81.6 and 20.3). **The commoner error runs the other way, though — declining a clear you were entitled to.** A man down you may fire it the length of the ice, the bench may sit legitimately empty until that expiry unless minors are stacked on the major, when a substitute is required immediately (NHL Rule 20.3), and a puck released before your penalty expires is not icing however late it lands. 🇬🇧 The IIHF book, which British hockey is played on, carries neither carve-out — **but seat a substitute in the penalty box immediately when a major empties it, because there the failure itself is a bench minor** (IIHF Rules 20.3 and 68.2).
...
11. **A net drive ends at the edge of the blue paint, clear of the red line around it — and short of the goaltender.** Screening him from outside the crease, without contact, is normally legal and worth doing; it is a skate inside the paint that risks the goal. **IIHF Rule 1.7** has it that *"[t]he marked line belongs to the Goal Crease"*, USA Hockey's Note to **Rule 625(b)** counts the crease lines in, and **Hockey Canada**'s Glossary agrees — but **the NHL writes no such sentence in its rule text, and the NHL's and the IIHF's own reference tables both allow the goal** to an attacker planted *"on the crease line"* (Situation 5 E; in the IIHF's Appendix IV that is Table 16 in the 2025/26 edition and Table 14 in the 2026/27 one). **So the IIHF answers the same play in two directions**, and staying off the line is the more conservative reading — the one that never has to be argued. ⚠️ **Losing the goal is the smaller half of the cost.** Contact you initiate with a goalie who is **in** his crease voids it whether or not it was incidental (**NHL and IIHF Rule 69.3**); **69.2** assesses a penalty for *"intentional or deliberate contact with a goalkeeper, whether or not the goalkeeper is inside or outside the goal crease, and whether or not a goal is scored"*; **USA Hockey 607(c)** gives *"[a] minor plus a misconduct or a major plus a game misconduct penalty"* for a check or charge on a goalkeeper *"within the goal crease or privileged area"*; and **Hockey Canada 8.5(b)** leaves no discretion at all — *"A Major penalty and Game Misconduct penalty will be assessed to any player who charges the goaltender."* **Two of those are ejections — USA Hockey's at its upper end, Hockey Canada's automatically.** ⚠️ **And the crease is not where **NHL and IIHF Rule 69.4** stops reaching you, either.** It also penalises an attacker whose *deliberate actions* prevent the goaltender from returning once he has left the crease to play the puck — a different foul from everything above, and one that does not wait for you to touch him. The clause itself turns on intent, not contact. [Offensive Zone Play](offensive_zone_play.md) sets out what the four books do with an attacker in the crease.
