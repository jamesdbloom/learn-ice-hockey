...
⚠️ **And that is only 20.4's *second* sentence. Its first adds a separate trigger:** an automatic game misconduct applies to any player *"assessed a **second major
...
**Reference Table 7 adds five more first-major ejection risks** — **boarding (41.4), charging
...
**How different is that from the NHL?** Materially — and note that the two books number their tables differently, so the Table 5 discussed here is *the NHL's*, not the IIHF's mentioned a moment ago. The NHL's own Reference Table 5 has
...
**Each of the Table 7 five does have a major one rung below it that carries no
...
- **SIHA-sanctioned games** run a different ladder: Level 2 minimum for junior and senior, **Level 1 minimum for recreational.**
