...
> | 1 · Repository | ✅ Done | Public at `jamesdbloom/learn-ice-hockey`; 39 documents in 8 sections; licences; agents and hooks in `.claude/` |
> | 2 · Infrastructure | ✅ Done | S3 + CloudFront, Terraform, GitHub OIDC — no long-lived keys |
> | 3 · Website | ✅ Done | Live at **learn-ice-hockey.com**, **53 pages** (`site/scripts/check-links.mjs` is the owner and prints the figure at every build; this row read **51** until 14 September 2026, two rows above a row the same release edited), Pagefind search, CI deploys on push to `main` |
> | 4 · Speech transform | ✅ Done | `md_to_speech.py`, all 39 documents, 83 self-test assertions, **0 unrecognised constructs** after round 22 |
...
> | 7 · Downloads and RSS | 🟡 **Text half done** | EPUB per layer + whole corpus, 8 PDFs (complete corpus 1,226 pp), per-document markdown, `/downloads` linked from every footer — all regenerated by `npm run build`. **Outstanding:** the audio containers and both RSS feeds, which need Phase 5 |
> | 8 · Offline PWA | 🟡 **Built, unverified on hardware** | Manifest, icons, generated service worker, 130 URLs / 6.9 MB precached, offline fallback page, installs to the home screen. **Outstanding:** the exit gate — a real iPhone and a real Android, aeroplane mode. Per-document offline controls deferred: with all text precached and no audio, a reader has nothing to choose |
>
> **What is actually left, 30 July:** the RSS feeds and audio containers in 7,
> real-device verification for 8, and 6 — plus 4b and 5 if narration is
> revisited. Everything blocking a reader from *reading* the corpus is done, and
> they can now take it away as EPUB, PDF or markdown, or install it.
>
> Two things landed alongside and are not phases: **Google Analytics** and
> **Search Console** domain verification, and the site's first **print
> stylesheet** — Cmd-P previously produced the navigation and sidebar.
...
NotebookLM has no API — this is manual.

⚠️ **This section originally suggested grouping by layer rather than per-document** (eight bundled episodes: Getting Started · Foundation & Rules · The Four Positions · Systems I (offensive) · Systems II (defensive) · Technique · Hockey IQ · Off the Ice), on the reasoning that the two-host format needs more material to work with. **That is not what was built.** `site/src/data/podcast.json` is per-document — one episode per page, 37 of 39 documents as of 17 September 2026 — and the site's audio player, RSS feed and download packaging (§10) are all built around that one-episode-per-page shape. **Decision, 17 September 2026: keep per-document.** Changing to layer bundles now would mean re-recording every existing episode and reworking the player/feed/download layer for no benefit the corpus has found a use for. `project/plans/PODCAST_AUTOMATION_LOCAL.md` and `.claude/skills/notebooklm-episode/` follow per-document; `scripts/podcast_queue.py` derives the episode order from `site/src/data/structure.json`, the same file that drives the site's own page order, so the two can never drift apart silently.
