...
- **Offside** requires **both skates** to completely cross the blue line before the puck completely crosses it. One skate on or behind the line keeps you onside. The puck must *completely* cross the line.
  ...
  **⚠️ The airborne trail skate is an NHL/IIHF rule and is the opposite under USA Hockey — always flag this.** Under the NHL a raised skate over the neutral zone keeps you onside. Under **USA Hockey Rule 630(a)** it does not: *"A player is considered 'offside' when the player does **not have skate contact** with any part of the Neutral Zone or the blue line when the puck crosses the determining edge,"* and *"A player only needs to have **skate contact** with one skate with the blue line to be considered 'onside.'"* So the trail-skate-drag technique this corpus teaches as a headline zone-entry skill is **offside under USA Hockey unless the skate is touching**. ⚠️ **AND IT IS TWO BOOKS OF THE FOUR, NOT ONE — added 2 September 2026 after verification in primary text.** **Hockey Canada Rule 6.11** carries the same limb **twice over**, which is why the corpus's *"two books of the four"* framing is correct and this passage was the incomplete one. The **preamble** (`hc_layout.txt:4205-4208`): *"Once the puck has completely crossed the blue-line, it is the position of the player's skates … **Only the player's skate(s) that are in physical contact with the ice surface will be used in determining an off-side.**"* And **Interpretation 3 to 6.11(a)** (`hc.txt:5478-5482`) rules the exact fact pattern: *"A player has one skate above the blue-line (not touching the ice) and one skate over the blue-line at the instant the puck completely crosses the blue-line. **OFF-SIDE.**"* Two independent limbs, not an inference.

  ⚠️ **QUOTE THE PREAMBLE, NEVER Note 1.** Hockey Canada's own **Note 1 to 6.11(a)** is loosely worded — *"A player is NOT off-side when any part of either of their skates, are in physical contact with, **or on their own side of the line**"* — which **read alone could be taken to permit an airborne skate**. The preamble and Interpretation 3 override it. **A reviewer quoting Note 1 in good faith would reach the opposite conclusion and be wrong.**

  Most North American rec and youth players are under USA Hockey rules. Wherever the technique appears, say which book it works in.
...
- **The trapezoid** (goalkeeper's restricted area) applies in the **NHL** and in **IIHF play** — both read directly in the rulebooks, at NHL Rules 1.8 and 27.8 and IIHF Rules 1.8 and 27.7. ⚠️ **The KHL is commonly said to have one too, and that is UNVERIFIED here: no KHL rulebook exists in any form in `sources/`** — the only support held is the IIHF's own 2021 announcement of its unified rulebook, calling the restricted area a *"trapezoid familiar to NHL and KHL"*, **which is a governing body's aside rather than a KHL rule.** ⚠️ **And neither adoption date is in any book on disk.** `2005` returns **zero** in both NHL extractions, flattened and de-hyphenated — every four-digit year in that book was enumerated — and `2021` returns zero in both IIHF editions. The NHL book *does* date rules when it wants to (9.6, 9.7, 28.6), and it **does not date this one**; the IIHF's 2021-22 date comes from the announcement, not the rulebook. **Carry the hedge and the limit whenever you carry the claim.** ⚠️ **Neither USA Hockey nor Hockey Canada writes a trapezoid at all — but how much rec, beer-league and youth hockey that adds up to is a question about LEAGUES rather than about BOOKS, and no document in this corpus has a count for it.** ⚠️ **THIS LINE PREVIOUSLY READ *"Most rec, beer-league and youth associations do not use it"* — a flat prevalence claim, unsourced, in the file that forbids exactly that.** It was caught on 3 September 2026 by an agent that had been instructed BY ITS BRIEF to import this sentence into `getting_started.md`, **declined, and cited the owner against it** (`goaltender.md:1368`, which says in terms that it *"has no count for"* it; `on_ice_communication.md:417` agrees). ⚠️ **The safe claim is about MARKINGS, which the corpus does hold** — `rink_map.md:115` *"Many rec rinks have no trapezoid at all"* and `uk_rules.md:196` *"so many British rinks have no trapezoid painted on them"*. **Say what is painted, not what proportion of associations do what.**
...
goal crease**"*), while **69.4 takes the GOAL outside the crease only for CONTACT** — ⚠️ **but its third paragraph penalises positioning alone, so never write that 69.4 reaches contact only**. ⚠️ **No provision of Rule 69 voids a goal for positioning alone from outside the crease, so
...
[`rink_map.md`](../content/foundation/rink_map.md), which already states it
...
sources/                 ← in the repository. Everything in it is gitignored for
    ...
    carha.txt            CARHA Hockey Rule Book — ⚠️ THE ADULT REC BOOK, and it was
                           MISSING from this list while the corpus quoted it. Found
                           2 September 2026 when an agent cited `carha.txt:3439`
                           (Rule 73(d), the two-line pass) and the list did not have it.
```

⚠️ **THIS LIST IS A SUMMARY AND IT GOES STALE. `ls sources/*.txt` returns 28 files; the list
above names 8.** `sources/README.md` is the authoritative inventory and outranks this cell —
**and `ls` outranks both.** ⚠️ **A hand-maintained inventory of a growing source set decays
silently, and this is the second such list in the repository caught short in two rounds** (the
other was `.claude/agents/rules-verifier.md`'s table, short by two). **Run the `ls`.**
...
| `Key:` | **A load-bearing fact about this section.** ⚠️ **The gloss here read *"the one thing to remember"* until 2 September 2026, and that was descriptively FALSE: ⚠️ **THE COUNTS THAT WERE HERE WENT STALE ON THE DAY THEY WERE WRITTEN, which is this guide's own recorded failure recurring inside the correction that records it.** They read *"96 of the 473 blocks using `Key:` carry two or more, 20 carry three or more"*. **Re-measured over the working tree hours later: 474 blocks use `Key:`, 97 carry two or more, 22 carry three or more.** ⚠️ **Off by 1, 1 and 2 — because they were measured against a corpus that was still being edited in the same round.** **State the SHAPE and name the COMMAND, never the totals:** many `Key:` blocks carry more than one value, a fifth of those carry three or more, and the maximum is seven. ⚠️ **Re-run rather than quote:** extract the ` ```facts ` lines, count `Key:` per block, and sort. **The seven, in `zone_entries.md`, reproduced exactly at both measurements — a maximum is stable in a way a total is not, and `zone_entries.md:55` carries SEVEN — all seven being separate qualifications on Tulsky's sample, every one of which must stay. ⚠️ **The spec was corrected to the corpus, NOT the corpus to the spec**, because enforcing "one" would have evicted caveats to satisfy a one-line table entry.** |
...
3. **Ownership applies to facts too.** A fact may not restate a number another
   ...
   dimensions belong to `rink_map.md`. Point, do not copy.
...
- **Define a term where it is used in a particular sense; point at the glossary for the rest.**
  [`rink_map.md`](../content/foundation/rink_map.md) is the dictionary for
...
| **Point** (the area **just inside** the blue line — ⚠️ **an area, NOT the line itself**; this cell said *"(blue line)"* until 2 September 2026 and **a writer working to spec reproduced the defect in six places across four documents, two of them voiced `facts` values**) | **Half-wall** (level with the faceoff dot) |
...
|---|---|---|
| Rink dimensions, lines, creases, faceoff spots, hash marks, circle radii | `rink_map.md` | NHL 200 × 85 ft; crease 8 ft wide × 6 ft deep; end-zone dots 20 ft from the goal line; hash marks 2 ft long, 5'7" apart, four per circle |
| Trapezoid geometry | `rink_map.md` | **NHL Rule 1.8**: *"eight feet (8') from each goal post"* at the goal line → *"eleven feet (11') from the goal post"* at the boards. With a 6 ft goal that is **22 ft and 28 ft**. ⚠️ **Never take dimensions from 27.8** — its 6-ft-from-post wording is pre-2014 and gives an 18 ft base. **IIHF Rule 1.8** gives **6.80 m and 8.60 m** — 22 ft 4 in and 28 ft 3 in. ⚠️ **The two books agree to within four inches and three inches: there is NO width divergence, and asserting one is a content critical in the document that owns regions.** **Where they DO differ is DEPTH**, and only because the goal line sits differently: NHL 1.5 puts it *"Eleven feet (11') from each end of the rink"*, IIHF 1.5 *"4.0m"* — about 2 ft 1 in deeper. ⚠️ **Both books number this rule 1.8, which is luck, not safety — name the book.** |
| Gap-control distances | `defending_the_rush.md` | **CANONICAL VALUE: 2–3 stick lengths in the neutral zone → a stick and a half at the red line → one stick length at your own blue line.** **THE FIVE STANDING RULES FROM THIS ROW — these are the reusable part; the round narrative is below them and you do not need it to apply them.** **(1) An absence claim and a contradiction are materially different disclosures and must not be written interchangeably.** *"No source was found"* and *"a published page prints a different number"* ask different things of a reader. **(2) Run any "no page prints X" census in at least three units — stick lengths, feet, metres — before writing the sentence.** A unit alone is enough to hide a source, and it has hidden two from this row. **(3) Measure propagation with the RUNG STRINGS, not with `stick length`.** Two documents match a naive search and carry no rung: `puck_support_and_spacing.md` (a SUPPORT distance, its own owner) and `faceoffs.md` (faceoff positioning, NHL 76.4 / USA Hockey 613(b)). **(4) The provenance limit travels with the rung.** The existing *"a guide, not a law"* hedge is about **applicability, not provenance**, and does not do that job. **(5) Never write that coaching guidance *commonly* puts the last two rungs anywhere** — that is a prevalence claim the owner cannot support — and never state the ladder as system-independent, because the owner itself says an aggressive press and a passive trap want different gaps. **A LANDMINE FOR THE NEXT CENSUS:** `sources/bvhs.txt:1457` reads *"maintain a consistent gap (roughly a stick's length away)"* and matches on both `gap` and `stick length`. **It is a GOALTENDER'S gap on a breakaway**, not a defenceman's rush gap — wrong population, not a ladder source. **NO COUNT OF CARRYING DOCUMENTS IS GIVEN HERE ON PURPOSE:** three agents produced three different ones in one round, and the pattern that produces a count is the pattern that produces false positives. **— THE ROUND NARRATIVE, kept because each entry records a failure this row committed —** ⚠️ **RETRACTED 8 September 2026: this row previously said only the first rung was sourced. That is FALSE — see below.** CrossIceHockey states *"In the neutral zone, aim to stay about two to three stick lengths from your opponent"*; **"stick and a half" returns zero from every book in `sources/` — **verified, all extractions.** ⚠️ **But an earlier version of this row also said "from every cited page", and that is FALSE:** the owner's own Sources trailer quotes HockeyShare verbatim — *"D1 tries to remain **1.5 stick lengths** away from the forward in the neutral zone and about 1 length in the defensive zone"* — refetched HTTP 200 on 2 September 2026. ⚠️ **THE ACCURATE SCOPE, AFTER A 26-PAGE SEARCH ON 8 SEPTEMBER 2026:** the blue-line rung **IS published and anchored to that line exactly** — HockeyShare's *editorial blog post* "Gap Control Basics" (bylined 3 May 2010) prints *"1 stick-length/body contact made at the defending blue line"* — and a second line-anchored page (Weiss Tech Hockey, bylined 3 Mar 2015) prints **two** there instead. ⚠️ **The red-line rung is CONTRADICTED, not unsourced: the only page found anchoring a distance to the centre red line prints TWO stick lengths, not a stick and a half.** An absence claim and a contradiction are materially different disclosures and must not be written interchangeably. ⚠️ **HockeyShare hosts BOTH a zone-anchored user-submitted drill AND the line-anchored blog post, and THEY DISAGREE — so "the HockeyShare page" does not identify a source.** The owner sets all of this out at `#target-distances`; its Sources trailer records the fetches. ⚠️ **Three of the four refuting domains were ALREADY IN THIS CORPUS'S OWN SOURCES TRAILERS** — re-tested at `HEAD~1` on 8 September 2026 and **UPHELD**: `hockeyshare.com`, `weisstechhockey` and `edgeiceacademy` were all already cited in `content/`; only `humankinetics` was new. ⚠️ **AND THE CORROBORATION IS THINNER THAN "published by more than one page" SOUNDS.** Refetched 8 September 2026: the CrossIceHockey article has **no visible author and no visible date** — stored author is a CMS account `production`, stored date 9 October 2025, and the site's stylesheet sets the byline and the post date to `display:none` — on a site that solicits guest and sponsored posts. The Elite Hockey Canada page **reproduces NHL Coaches' Association clinic material** (David Quinn, 2019 NHLCA Global Coaches' Clinic, *"Copyright NHL Coaches' Association, Inc."*, plus *"Credits: Mario DuHamel / Jack Han"*) alongside its own prose, so it is not clearly the originator. **Both facts are now in the owner. Count pages if you like, but weigh them before writing "published".** ⚠️ **A THIRD CORRECTION, IN THE DIRECTION THIS GUIDE GUARDS LEAST WELL: the owner's syndication caveat on Cutting Edge Ice Academy was WRONG and has been replaced rather than deleted.** It read the identical article at `kingcobrashockey.com` as a reason to doubt Edge Ice Academy's authorship. Both footers carry **the same street address**, 71 Midland Ave, Elmwood Park, NJ — a rink and its resident club, one publisher. **The duplication was never evidence against authorship**, and the caveat made the corpus's own source look weaker than it is. The real limit, now stated instead, is that the piece carries no byline. ⚠️ **And one coaching book sets the gap against the FORWARDS rather than against the LINES** — ⚠️ **an earlier version of this line said it *"rejects the ladder's SHAPE"*, and so did the owner. That was stronger than the source.** The two sentences quoted are about where gap control BEGINS, and are compatible with a three-rung ladder; **the sentence that genuinely differs is a third one the row had not carried** — the excerpt asks a defenceman to *"maintain the good gap from blue line to blue line"*, against the owner's *closing the whole way in*. **That is the divergence worth carrying, and it is now in the owner's body.** The book: a Human Kinetics book excerpt (credited on the page to Joseph Bertagna; chapter author not named there) holds that *"the blue line does not determine where gap control begins"* and **prints no distance anywhere** — negative re-tested 8 September 2026 with feet, foot, stick length, metre, meter, yard and inch all returning 0, against the same positive control of 33 occurrences of "gap", which reproduces exactly. ⚠️ **THE CHAPTER AUTHOR IS RECOVERABLE AND HAS NOW BEEN RECOVERED — this was a disclosure that made the corpus look LESS supported than it is, one link from the page it disclaimed.** The publisher's own product page (ISBN 9780736062015, ©2016) shows a collection of **sixteen chapters, each by a different coach or coaches**, and gives **Chapter 8 "Skills for Defensemen" to JACK PARKER**, Bertagna keeping Chapter 10 on goaltending. ⚠️ **Do NOT write "sixteen chapters by sixteen coaches" — the contents list runs to SEVENTEEN names.** Chapter 14 is credited to George Gwozdecky *and* Michael Zucker, and Zucker is not among the sixteen the description copy names. The owner's body argued from this list, so the list has to be described as it is. The excerpt numbers its figures **8.1–8.3**. ⚠️ **That is an INFERENCE from figure numbering, and it must stay visibly an inference in the owner's text.** Control: the sibling excerpt "Practicing the Power-Play Breakout" numbers figures **14.13–14.16** — an earlier version of this line said 14.15; 14.16 is on the page — and chapter 14 is "Special Teams and Situations", so figure number tracks chapter. ⚠️ **A SECOND CORRECTION IN THE SAME PLACE — AND IT WAS ITSELF WRONG, RE-CORRECTED 8 SEPTEMBER 2026.** The owner's trailer had said the page credited an *editor*; it does not, and the correction then said the page *"labels him Author"*. **It does not do that either.** ⚠️ **TWO DIFFERENT PAGES:** the **excerpt** page prints only *"This is an excerpt from The Hockey Coaching Bible by Joseph Bertagna"* and names no chapter author; the string *"Author: Joseph Bertagna"* is printed on the publisher's **PRODUCT** page. The label migrated between the two in the writing, and both this row and the owner carried it. **Say WHICH page prints WHAT, and say "printed"** — `edited` does occur once in the excerpt page's raw HTML, inside a review-widget JS string, so a checker grepping the source rather than the rendered text will contradict a bare "zero occurrences" claim. ⚠️ **The general lesson is the dangerous one: a wrong correction reads as doubly settled, because the reader can see that somebody already checked.** The volume genuinely is multi-contributor, but that comes from the CONTENTS LIST, not from a label either page carries. ⚠️ **This row caught a far miss (an IIHF metric figure) and left a near one standing on a page in its own owner's trailer — rule 6 failing inside the guide that states it.** ⚠️ **BUT THAT IS A PHRASE CENSUS AND IT MISSED A METRIC ONE.** `iihf_coachdev_off_tactics.txt:1671` — IIHF Coach Development Level I, a §21 drill sheet — prints *"Defender shadows movement of attacker while **maintaining a gap of 2.5-3 m**."* A governing-body gap figure, **in metres, so `stick length` returns 0 from that file while `gap` returns 7**. ⚠️ **It is a shadow-SKATING drill rather than a rush read, and converting its metric spacing into stick lengths and calling it the source would be exactly the derived claim this guide warns against — so it was reported and NOT written in.** The lesson is the census's, not the ladder's: **a unit is enough to hide a source.** ⚠️ **IT THEN HAPPENED A SECOND TIME IN THE SAME ROUND, and this time the hidden source was IN THE OWNER'S OWN SOURCES TRAILER.** `howtohockey.com/playing-the-angles-in-defense/` — cited by `defending_the_rush.md` for the boardside shoulder — continues that very sentence *"aslo make sure you are within 10 feet"* (sic; refetched HTTP 200, 8 September 2026). A published coaching gap distance **in FEET**, invisible to a `stick length` census, on a page the document already cites. It is **not line-anchored**, so it does not disturb the red-line claim, and it is now recorded in the owner with that qualification. **Those two misses are what standing rules (2) to (5) at the head of this row were written from; the bvhs.txt landmine and the no-count instruction are there too, and are not repeated here.** The `bvhs.txt:1457` entry's surrounding checklist, for anyone who needs to confirm the population, is *"The Challenge / Gap Control & Retreat / Read the play / Push to the Angle"* — a goalie backing in. Every document carrying a rung of this ladder points here as owner. ⚠️ **ROUND 69 MOVED THE OWNER'S BODY PROVENANCE BELOW THE THREE RUNGS. ⚠️ **AND THE FIGURES THIS ROW CARRIED FOR THAT WERE FALSE, WHICH IS THE MORE USEFUL HALF OF THIS ENTRY.** It said the provenance *"collapsed from 5,488 characters to 2,483"* and that `Important.` flags went from four to zero. Measured across every commit that touched the file, **§Target distances went 3,103 chars → 5,209 and its warning flags went 1 → 4**: it roughly DOUBLED. ⚠️ **The 5,488 was measured against the WORKING TREE mid-round, after a gap-provenance repair had already added ~2,400 characters that the same round commissioned — so the "reduction" was against an uncommitted intermediate state the author had created, and no committed state ever held it.** In both the parent and the child a listener reaches a number in the facts block before any provenance at all, so *"four flags deep before the reader reaches a number"* described no state of this repository either. **The reordering is real and should stay; the measurement was not. Measure a change against HEAD, never against a working tree you have been editing all session.**** The page-by-page grading — bylines, CMS accounts, the NHLCA clinic block, the syndication question — now lives in the owner's Sources trailer, which is the layer this guide names for it; the body keeps only what changes what a reader does. **Nothing was deleted, and a future author must not reinstate the grading in the body: it is voiced aloud, four `Important.` flags deep, before the reader reaches a number.** |
...
| Positional workload | `positions/center.md` | ⚠️ **THIS ROW WAS STALE AGAINST ITS OWN OWNER and is corrected 2 September 2026 — the specification was MORE CONFIDENT than the document it names as owner, which is the failure this guide records about itself twice.** It read as though the territory claim were established. **It is not: nothing measures how much OF the ice a position covers.** The owner now says *"Read that half as coaching craft too, not as a finding"* and labels it *"coaching craft rather than a measured comparison"*, scoping the absence as *"no measurement of how much of the ice a position covers was found for this document."* ⚠️ **Carry the LABEL, not just the claim.** A centre works more *of* the ice (three zones, full width) — **not** more distance, and **neither half is measured**: the territory half is untested, and the distance half is tested and points the other way. ⚠️ **AND A THIRD CORRECTION, 2 September 2026, in the OPPOSITE direction to the first: this row said only *"no study separates centres from wingers on distance"* and left a reader to infer the data does not exist. IT DOES.** The NHL publishes **per-skater skating distance AND shorthanded ice time**, each against a position code separating C from L and R — verified live at `api-web.nhle.com/v1/edge/skater-detail/…` and `api.nhle.com/stats/rest/en/skater/timeonice?…`. ⚠️ **So the correct claim is that the comparison is UNRUN, not impossible** — and the reason it stays unrun is a real one that must travel with it: **a distance total accumulates over every second on the ice, so a raw centre-versus-winger gap carries ice time as well as workload.** `distancePer60` exists in the payload and would control for it. ⚠️ **NOBODY HAS COMPUTED EITHER SPLIT FOR THIS CORPUS, AND NOBODY SHOULD WITHOUT DECIDING FIRST what sample and season coverage would make it defensible — nothing mechanical would stop a naive per-60 figure entering the corpus and passing every gate.** Lignell et al. 2018: defencemen cover **29% more** total skating and are on the ice **47% longer**; forwards do **54% more** high-intensity skating per minute. No study separates centres from wingers on distance |
...
| **Power-play conversion (21.1%)** | ✅ **SETTLED 22 September 2026, by RECOMPUTATION rather than by argument.** **OWNER: `hockey-iq/risk_management.md`. CANONICAL ESTIMATOR: the LEAGUE AGGREGATE** — goals ÷ opportunities, **every power play weighted once**. Both NHL reports were refetched live that day: `team/powerplay` gives **1,595 ÷ 7,555 = 21.1118% → 21.1%**; the `team/summary` report the other document names gives an **unweighted mean of the 32 clubs' own `powerPlayPct` = 21.1044% → 21.1%**. ⚠️⚠️ **THE COINCIDENCE IS REAL AND THINNER THAN ONE DECIMAL MAKES IT LOOK: 21.10% against 21.11% — they agree at ONE decimal and SEPARATE AT TWO.** ⚠️ **And they coincide THIS SEASON ONLY because opportunity volume happens not to correlate with conversion; a season where the busiest power plays were also the best ones would pull them apart.** **Why the aggregate is canonical, stated in the owner's own trailer and not only here:** the corpus's question is *"what does one more power play against me cost?"*, which weights POWER PLAYS, not clubs. **The mean of 32 club percentages answers a different question — *"what does a typical club convert at?"* — and is legitimate for the team-level survey it lives in. NEITHER WAS DELETED; both are now named at two decimals with the recomputation date, so nobody re-derives them.** ⚠️ **The ASYMMETRY this row already recorded still holds and is why it stayed invisible: the owner's figure is BODY PROSE and is VOICED; the mean sits in a Sources trailer, which `md_to_speech` DROPS.** ⚠️ **AND A RENDERER TRAP FOUND WHILE CHECKING IT: the speech layer verbalises figures, so the string to search in SSML is `twenty-one`, NOT `21.1` — a naive grep returns zero and looks clean.** | **Nothing owed. Do not re-derive either figure; quote the owner and say which estimator.** ⚠️ **This row previously cited `risk_management.md:542` and `how_to_watch_hockey.md:613`/`:648` — ALL THREE STALE. The body statement is in §*The penalties that are not trades at all*, the owner's provenance in its Sources trailer, and the mean form in `how_to_watch_hockey.md`'s Sources trailer. SECTION NAMES, NOT LINE NUMBERS, per this file's own lesson.** |
...
| **Concussion red-flag COUNTS — and there are TWO lists, not one** | **CRT6's ten: `off-the-ice/conditioning_and_recovery.md`. The British fourteen: `foundation/uk_rules.md`.** ⚠️ **`technique/body_contact_and_battles.md` cites BOTH and owns NEITHER — it must always name whose count it is quoting.** | ⚠️ **THIS ROW EXISTS BECAUSE THREE DOCUMENTS STATED ONE OF THESE COUNTS WITH NOTHING WATCHING THEM, and on 19 September 2026 a bare *"the ten red flags"* was found in a document that serves British readers — a definite description carrying one list's number with the other list's audience.** **CRT6 (Echemendia RJ et al., *Br J Sports Med* 2023;57:692-694) lists TEN. The UK Concussion Guidelines for Non-Elite (Grassroots) Sport list FOURTEEN, and England Ice Hockey has formally adopted them.** ⚠️ **NEVER write a bare count. Name the list.** **The two DIVERGE in three places and the direction matters both ways: UKCG fires EARLIER on the skull (*"[a]ny suspicion of a skull fracture (e.g. cut, bruise, swelling, severe pain at site of injury)"* against CRT6's *"Visible deformity of the skull"*) and on seizure (it reaches limb twitching and tonic posturing); CRT6 fires LOWER on the neck (*"Neck pain or tenderness"* against *"Severe neck pain"*).** ⚠️ **AND CRT6 IS NOT SILENT ON DECREASED SENSATION — its *"Weakness or numbness/tingling in more than one arm or leg"* covers it, limb-qualified. The true contrast with the British *"[a]ny new neurological deficit"* is SCOPE, not ABSENCE; asserting absence is a live defect this row was written from.** **The reconciliation the corpus applies — *where the two differ, act on whichever fires first* — is THE CORPUS'S OWN SYNTHESIS and must be labelled as one: measured 19 September 2026, `whichever` returns 0 in the guidance whitespace-stripped, and UKCG never references CRT6 or SCAT at all.** ⚠️ **The fourteen ARE edition-stable — the April 2023 edition England Ice Hockey's own page still links was fetched and compared positionally, and all fourteen entries plus the instruction sentence are BYTE-IDENTICAL to November 2024 — so do NOT add an edition caveat to this list.** ⚠️ **Reading the source: the red-flag panel is TWO-COLUMN and interleaves in extraction (bullets at character offsets 41 and 82), its heading extracts letter-spaced as `R E D F L AG S`, and a naive whitespace-stripped phrase search over it produces EIGHT FALSE ABSENCES OUT OF SIXTEEN. Read it positionally; see `sources/README.md`.** ⚠️ **`sources/crt6.txt` is 1,535 bytes of page furniture from a 6.3 MB PDF — the tool is IMAGE-ONLY. `pdftoppm` it and read the pages; four separate reports have wrongly concluded it cannot be verified.** |
...
| **Goal-location figures (26.8% / 51.6%)** — share of goals scored within two stick-lengths / within twenty feet | `technique/body_contact_and_battles.md` | ⚠️ **ENTERED THE TABLE 2 September 2026 because it had NO ROW while being restated in at least one other document.** Computed for this corpus **from the NHL play-by-play feed, 400 games of 2024-25, 2,498 goals with coordinates**. **That population is part of the fact.** It is a corpus computation, **not a published figure**, and the label travels with the number. ⚠️ **THIS ROW UNDER-STATED ITS OWN BORROWER SET AND CERTIFIED A DOCUMENT THAT DID NOT MEET ITS STANDARD.** It named two sites; there are **four** — the owner, `rink_map.md:320`, `risk_management.md:71` and `defensive_zone_coverage.md:44`. It then declared `defensive_zone_coverage.md` to *"carry it correctly"* while that document, and `risk_management.md`, gave only *"400 games of 2024-25"* and **dropped the 2,498-goal population this very row calls part of the fact.** ⚠️ **The omission is not cosmetic and that is why it was repaired rather than the standard relaxed: "400 games" reads as the whole sample, while "the 2,498 goals in it that carry coordinates" reveals a FILTER — so dropping it makes the population sound MORE complete than it is**, which is the direction this project guards hardest against. **Both were repaired 3 September 2026; all four sites now carry the filter.** ⚠️ **Found by `rules-verifier` while checking something else entirely — it was sent to verify a stick dimension and reported the ownership gap in passing.** |
| **Penalty-call figures (2,967 / 265 / 16.6% / 12.1% / 12.0% / 71.4%)** — penalties counted, games, and the share taken by each of seven infraction types | `technique/body_contact_and_battles.md` | ⚠️ **ENTERED THE TABLE 11 September 2026, and this is the THIRD time this row's own warning has come true** — *a row per DOCUMENT is not a row per FIGURE*. Its sibling row above gave this document a row for the goal-location figures; these figures had none, and five sites in `foundation/rules_primer.md` aggregated them into a claim the owner never made. Computed for this corpus **from the NHL play-by-play feed, 400 games of 2024-25, 2,967 penalties**. **The seven types account for 71.4% between them and THE REST WERE NEVER BROKEN OUT** — that residue is part of the fact and must travel with the numbers. ⚠️⚠️ **THESE SEVEN FIGURES MAY BE RANKED ONE AGAINST ANOTHER AND MAY NOT BE ADDED TOGETHER.** Tripping, the largest at 16.6%, is a stick offence in **no** book. ⚠️ **The decisive evidence is NOT Rule 57.1's list of means, which is only an inference: it is NHL Rule 23.5's explicit "Stick Infractions Category" (`nhl_rules_layout.txt:2409-2414`) — butt-ending, cross-checking, hooking, slashing, spearing — which DOES NOT CONTAIN TRIPPING. That is the NHL classifying its own rules.** Checked in all five books the corpus cites: the IIHF's `SECTION 08. STICK INFRACTIONS` is 58-62 only; Hockey Canada's `SECTION 9 – STICK FOULS` is 9.1-9.5 only; USA Hockey files tripping under 639 and its Rule 411 list is *"aggressive infractions"*, not stick ones; CARHA has no stick category at all. So grouping it with stick fouls gives 42.5% to 28.9% and leaving it out gives 25.9% to 45.5% (⚠️ **45.5, and the owner document said 41.5 — an arithmetic slip found by the BORROWER agent on 11 September 2026 and relayed into this row unchecked by the coordinator before it was caught. 28.9 non-stick counted + 16.6 tripping = 45.5. The ruling is unaffected: both readings flip the ranking, which is the whole point**). **The same seven figures answer "are stick fouls the biggest group?" both ways, which means they do not answer it.** ⚠️ **The pattern to watch for is NOT the phrase "largest group": it is ANY sentence adding two or more of these seven together, whatever it calls the sum.** ⚠️ **AND THE COMPUTATION CANNOT BE RE-DERIVED FROM THIS REPOSITORY** — no script, no cached JSON, no stored extraction; `grep` for `2967` / `2,967` / `gamecenter` across `scripts/`, `project/` and `sources/` returns only prose restatements. So the residue question is answerable from the data and unanswerable from here. |
| **Post-shot hazard figures (11.4% / 19.7%)** — share of goals scored within three / within ten seconds of a previous shot | `systems/offensive_zone_play.md` | ⚠️ **ALSO ADDED 2 September 2026, and note WHY it was missed: this table already had a row for `offensive_zone_play.md`'s PRE-shot pass values, so the document looked owned.** A row per *document* is not a row per *figure*, and the second figure-set sat unowned behind the first. **Corpus computation with its stated basis; carry the label.** |
| **Rebound recovery (~60%)** | `systems/offensive_zone_play.md` | ⚠️ **ADDED 2 September 2026 — the THIRD figure-set in ONE document to be found without a row, and the second found AFTER the row above was written to record exactly this trap.** ⚠️ **THIS ROW WAS WRONG WHEN WRITTEN, AND `source-verifier` FETCHED IT THE SAME DAY.** It said *"Attributed to **Alex Novet, via The Coaches Site**"*. **The measurement is Novet's; the SENTENCE and the NUMBER are Greg Revak's.** ⚠️ **Novet's chart never prints 60%** — it prints *"% of Those Rebounds Recovered by Offense"* at **38–42% across all eight zones**; the ~60% is Revak's **complement arithmetic** and his aggregation of eight zones into one figure. **Primary: Alex Novet, *Novet Hackathon — CBJHAC 2020 v2*, Columbus Blue Jackets Hockey Analytics Conference 2020** (a hackathon deck, not a paper), restated by **Greg Revak, *Hockey IQ Newsletter*, 20 September 2020**, republished by The Coaches Site. ⚠️ **The corpus ALREADY uses the correct construction for this same author three times** — `shooting.md:412`, `offensive_zone_play.md:61` and `time_and_space.md:128` all say *"Greg Revak's readings off…"*. **This is the one figure from Revak's writing where the corpus omits him.** ⚠️ **AND NO LEAGUE, SEASON OR SAMPLE SIZE IS STATED ANYWHERE REACHABLE** — not on the chart, not in either article. **The deck is 403 from this machine's egress proxy (Zscaler), NOT link rot; its existence is confirmed via the Internet Archive.** ⚠️ **A representativeness check PASSED and is worth recording: recovery is flat at 38–42% across all eight zones, and Revak says so in terms — *"despite where the shot that created the rebound started from"* — so a single ~60% is a fair summary, not a cherry-picked row.** It appears at `:488` (**facts layer, voiced alone**), `:494` (body) and `:1059` (Key Takeaway 7), and **nowhere else in `content/`** — censused. ⚠️ **The attribution has NO INLINE LINK: the URL is in the Sources trailer, which `md_to_speech` DROPS**, so a listener gets the figure and the name and no way to reach either. ⚠️ **IT HAS NOW BEEN FETCHED, AND THIS CLAUSE IS THE SURVIVING FRAGMENT OF THE PRE-REPAIR TEXT — it said the opposite of the sentence above it, which is how a verified number gets re-hedged or cut. `commit-gate` caught it: a repair that reached half a cell.** The article, the chart image and the prior newsletter publication were all retrieved 2 September 2026 and are recorded in `project/verification/link_baseline.tsv`. ⚠️ **"A row per DOCUMENT is not a row per FIGURE" — that sentence was added to the post-shot-hazard row above for this reason, and a third figure in the same document still slipped past it.** |
...
Found 21 September 2026, in two sibling documents fixed days apart: `faceoffs.md` carried *"they are the only moments where you know in advance where the puck will be and when,"* and `offensive_zone_play.md` carried *"An offensive-zone faceoff is the only moment in the game where you get to arrange all five players exactly where you want them, with the puck about to arrive at a known place."* Neither claim is sourced, and neither is false enough to cut under non-negotiable 3 — both are genuine coaching observations about a real tactical feature. The corpus's standard move for this shape, seen throughout `off-the-ice/practice_and_development.md` and `off-the-ice/how_to_watch_hockey.md`, is to append a disclosure naming the claim as judgement rather than measurement (*"a coaching observation rather than a measured finding," "a coaching estimate rather than a counted one"*) — and that pattern is correct for a claim like *"most players practise shooting most because it is fun"* or *"most players' shifts run ten to twenty seconds longer than they believe,"* because the hedge directly qualifies the word doing the asserting (*most*, *longer*).

**"Only" is a different word from "most" or "longer," and a trailing hedge does not qualify it the same way.** "Only" asserts there is nothing else with the property — a bright-line existence claim — and appending *"a coaching observation, not a measured claim"* after the sentence has already said "only" leaves the absolute standing in the reader's or listener's ear before the hedge arrives. `faceoffs.md`'s fix did exactly this (kept "only," hedged a trailing clause) — not wrong, but weaker on this reasoning than the alternative below. `offensive_zone_play.md`'s fix instead reworded "only" itself to a bounded comparative ("as close as the game gets to a moment where...") before appending the same style of disclosure — see [`project/reviews/offensive_zone_play_only_moment_hedge_2026-09-21.md`](reviews/offensive_zone_play_only_moment_hedge_2026-09-21.md) for the worked example, and [`project/reviews/faceoffs_and_rules_primer_opening_rewrites_2026-09-21.md`](reviews/faceoffs_and_rules_primer_opening_rewrites_2026-09-21.md) for the earlier, weaker fix — not itself wrong enough to reopen, but not the pattern to repeat.

**So: when the unsourced word is itself an absolute** ("only," "always," "never," "nothing else") **reword that word to a bounded comparative first, then append the standard disclosure.** When the unsourced word is a ranking or a magnitude ("most," "longer," "rarely"), the standard trailing disclosure is sufficient on its own, as it already is throughout this corpus.
...
⚠️ **RETRACTED 2 September 2026 — this list cited, as an exemplar of a SOUND disclosure, one the corpus retracted in the same round.** It read that *"2-2-1" is genuinely not a hockey forecheck name*. **A search returned a hockey coaching page titled "2-2-1 Pressure Forecheck"**, and the unfalsifiable absolute was removed from `forechecking_systems.md`, which now reads *"is not what coaching material **standardly** calls this in hockey."* ⚠️ **An agent working to this spec would have reinstated the absolute** — which is the guide-goes-stale-against-its-own-corpus shape this file records about itself three times, occurring a fourth time in a list whose whole purpose is to name disclosures that held.
...
⚠️ **ONE known collision with key (A), not two — corrected 2 September 2026 by rendering the sheet.**
A circled **`C`** is (A)'s **coach**. ⚠️ **There is NO assistant symbol on that sheet.** An earlier version
of this passage said *"a circled `A` is its assistant"*, and the same claim is in
`.claude/agents/diagram-reviewer.md`. **The sheet has seventeen rows and no assistant** — `Ⓒ Coach`,
`G Goaltender`, `● ○ Forward / Player`, `▲ △ Defender / Player`, dashed end-position, forward skating,
backward skating, lateral crossovers, pivoting, skating with control of the puck, passing, shooting,
