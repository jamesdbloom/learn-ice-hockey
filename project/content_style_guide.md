...
| **Power-play conversion (~21.1%)** | ⚠️ **NO OWNER — and TWO ESTIMATORS agreeing to one decimal by coincidence of rounding.** `hockey-iq/risk_management.md:542` computes it as a **league aggregate** (1,595 power-play goals ÷ 7,555 opportunities = 21.11%); ⚠️ **`off-the-ice/how_to_watch_hockey.md:613` IS A STALE POINTER — corrected 2 September 2026.** That document's BODY no longer prints the figure at all; it survives only at **`:648`, inside the Sources trailer**, as *"power-play percentage mean 21.1%"* — the **MEAN OF 32 TEAM PERCENTAGES**. ⚠️ **Those are different quantities that happen to round the same way.** ⚠️ **And note the ASYMMETRY that makes this less urgent than the row first claimed but harder to see: `risk_management.md:542` is BODY PROSE and is VOICED; the `how_to_watch_hockey.md` figure is in a Sources trailer, which `md_to_speech` DROPS. So a listener meets one estimator and never the other, and a reader on the site meets both — which means the divergence is invisible in the layer where it would be heard, and only a reader comparing two documents could ever notice it.** | **Decide an owner and record WHICH ESTIMATOR is canonical.** ⚠️ **This is the pre-divergence state this table exists to prevent: the next writer to recompute either way produces a third figure and has nothing to check it against — and the corpus has already had one figure sit wrong in three files at once because all three printed the operands and none checked the arithmetic.** |
