...
- **After your own icing:** play stops, defensive-zone faceoff, the offending team **cannot change lines** and **cannot use a timeout**.
...
- **Power play formations:** the **1-3-1** has exactly **one** player at the point (one point, two half-walls, one bumper between them, one net front) — most first units use four forwards and one defenceman. The **umbrella** has three high. The **overload** loads four onto one side. There is no bumper in a true umbrella.
...
| Zone-entry values | `zone_entries.md` | Tulsky et al., MIT Sloan 2013, data 2011-12: 0.53–0.62 shots per carry-in vs 0.22–0.28 per dump-in; net values +0.42 carry attempt vs +0.12 dump-and-chase |
| Breakout options and their order | `breakouts.md` | Nine named options (up, wheel, reverse, D-to-D, rim, over, quick-up, stretch, off the glass). Each is keyed to a **read**, not to a rank — the document asserts no fixed preference order, so do not invent one elsewhere |
